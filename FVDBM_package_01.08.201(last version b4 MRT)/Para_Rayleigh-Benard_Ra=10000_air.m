...
FCD=0; %%% Flag for whether the problem is purely heat conduction. 0----No; 1----Yes
d=2;
qh=9;
qt=9;
if d~=2
...
% Thermal
T_bc=[1/3,1/3,2/3,1/3,2/3];  % Temperature [top,right,bottom,left,immersed]
...
Body_force=[0;-0.04]/7100*10000*2*(Ra/10000);
Pr=(Tau/3)/(Tau_t/3*2)
Ra=abs(Body_force(2))*(T_bc(3)-T_bc(1))/T_ref*(Y2-Y1)^3/(Tau/3)/(Tau_t/3*2)