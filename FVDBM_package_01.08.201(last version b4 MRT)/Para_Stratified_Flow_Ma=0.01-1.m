...
Rho_out=[1.9,2,2,2]; % Outlet density [top,right,bottom,left]
...
% Manual change for top-right corner
ND=NODE{N_I+N_L-1};
ND{2}=31;
bc_macro=ND{21};
bc_macro(1,1)=Rho_out(1);
ND{21}=bc_macro;
NODE{N_I+N_L-1}=ND;
% Manual change for bottom-right corner
ND=NODE{N_I+N_L-1+N_H-1};
ND{2}=31;
bc_macro=ND{21};
bc_macro(1,1)=Rho_out(3);
ND{21}=bc_macro;
NODE{N_I+N_L-1+N_H-1}=ND;
%% Control Flags
...
NOut=3100; % 3100 for extrapolated u and v; 3199 for extrapolated u, v=0; 3099 for both density and velocity prescribed
NStw=410;
NMow=510;
NWdp=620;
NIof=800;
