function [CELL,NODE,FACE]=stencil(CELL,NODE,FACE,V1,V2,N_I,N_I_N,N_L,N_H,h,dx,dy,X,Y,X1,X2,Y1,Y2,dt,FM)
    ...
    if FC{24}==2 || FC{24}==3
        ...
        for k=1:q1
            ...
            elseif FC{24}==2
                ...
                if single(s30(1,k))<X1
                    s30(1,k)=s30(1,k)+(X2-X1);
                end
                if single(s30(1,k))>X2
                    s30(1,k)=s30(1,k)-(X2-X1);
                end
                if single(s30(2,k))<Y1
                    s30(2,k)=s30(2,k)+(Y2-Y1);
                end
                if single(s30(2,k))>Y2
                    s30(2,k)=s30(2,k)-(Y2-Y1);
                end
            else
                error('Logic Error!');
...
%             end
        ...
        for k=1:q1
            fs_d=fc26(:,2);
            fs_u=fc26(:,3);
            point_swtich_counter=0;
            if L_V_1(k)>=0
                s31(k,1)=dis(s30(:,k),fs_d)/(fc27(2)+fc27(3));
                if s31(k,1)>1
                    if single(fs_d(1,1)-fs_u(1,1))==0
                        if fs_d(2,1)<s30(2,k)
                            s31(k,1)=dis(s30(:,k),fs_d+[0;Y2-Y1])/(fc27(2)+fc27(3));
                        else
                            s31(k,1)=dis(s30(:,k)+[0;Y2-Y1],fs_d)/(fc27(2)+fc27(3));
                        end
                    elseif single(fs_d(2,1)-fs_u(2,1))==0
                        if fs_d(1,1)<s30(1,k)
                            s31(k,1)=dis(s30(:,k),fs_d+[X2-X1;0])/(fc27(2)+fc27(3));
                        else
                            s31(k,1)=dis(s30(:,k)+[X2-X1;0],fs_d)/(fc27(2)+fc27(3));
                        end
                    else
                        error('Logic Error!');
                    end
                    point_swtich_counter=point_swtich_counter+1;
                end
                
                
                s32(k,1)=dis(s30(:,k),fs_u)/(fc27(2)+fc27(3));
                if s32(k,1)>1
                    if single(fs_d(1,1)-fs_u(1,1))==0
                        if fs_u(2,1)<s30(2,k)
                            s32(k,1)=dis(s30(:,k),fs_u+[0;Y2-Y1])/(fc27(2)+fc27(3));
                        else
                            s32(k,1)=dis(s30(:,k)+[0;Y2-Y1],fs_u)/(fc27(2)+fc27(3));
                        end
                    elseif single(fs_d(2,1)-fs_u(2,1))==0
                        if fs_u(1,1)<s30(1,k)
                            s32(k,1)=dis(s30(:,k),fs_u+[X2-X1;0])/(fc27(2)+fc27(3));
                        else
                            s32(k,1)=dis(s30(:,k)+[X2-X1;0],fs_u)/(fc27(2)+fc27(3));
                        end
                    else
                        error('Logic Error!');
                    end
                ...
                s31(k,1)=dis(s30(:,k),fs_u)/(fc27(2)+fc27(3));
                if s31(k,1)>1
                    if single(fs_d(1,1)-fs_u(1,1))==0
                        if fs_u(2,1)<s30(2,k)
                            s31(k,1)=dis(s30(:,k),fs_u+[0;Y2-Y1])/(fc27(2)+fc27(3));
                        else
                            s31(k,1)=dis(s30(:,k)+[0;Y2-Y1],fs_u)/(fc27(2)+fc27(3));
                        end
                    elseif single(fs_d(2,1)-fs_u(2,1))==0
                        if fs_u(1,1)<s30(1,k)
                            s31(k,1)=dis(s30(:,k),fs_u+[X2-X1;0])/(fc27(2)+fc27(3));
                        else
                            s31(k,1)=dis(s30(:,k)+[X2-X1;0],fs_u)/(fc27(2)+fc27(3));
                        end
                    else
                        error('Logic Error!');
                    end
                ...
                s32(k,1)=dis(s30(:,k),fs_d)/(fc27(2)+fc27(3));
                if s32(k,1)>1
                    if single(fs_d(1,1)-fs_u(1,1))==0
                        if fs_d(2,1)<s30(2,k)
                            s32(k,1)=dis(s30(:,k),fs_d+[0;Y2-Y1])/(fc27(2)+fc27(3));
                        else
                            s32(k,1)=dis(s30(:,k)+[0;Y2-Y1],fs_d)/(fc27(2)+fc27(3));
                        end
                    elseif single(fs_d(2,1)-fs_u(2,1))==0
                        if fs_d(1,1)<s30(1,k)
                            s32(k,1)=dis(s30(:,k),fs_d+[X2-X1;0])/(fc27(2)+fc27(3));
                        else
                            s32(k,1)=dis(s30(:,k)+[X2-X1;0],fs_d)/(fc27(2)+fc27(3));
                        end
                    else
                        error('Logic Error!');
                    end
            ...
            if FC{24}==2
                if point_swtich_counter~=1
                    error('Logic Error!');
                end
            elseif FC{24}==3
                if point_swtich_counter~=0
                    error('Logic Error!');
                end
            else
                error('Logic Error!');
...
%   Type 3: the cell that has one and only one face attached to the inner boundary.
    ...
    % The following part is commented because the provided algorithms for
    % 2nd-order mapping are nothing working until Xiaodeng's algorithm is
    % provided
    
%     %% Algorithm 1 see paper Maik Stiebler, Jonas Tolke, Manfred Krafczyk. An upwind discretization scheme for the finite volume lattice. Computers & Fluids, see CELL v04.25.2017
...
% %     end
%     
%     %% Algorithm 3 weighted scheme based on the distance to the center
%     centroid, this one FAILS
%     %% Fill CL{38~43} for cell type 1 and CL{44~50} for cell type 2 & 3
% %     for i=1:M
% %         CL=CELL{i};
% %         if CL{37}==1 % cell type 1
...
% %         end
% %     end
    %% Algorithm 4 Xiaofrng's 2nd-order mapping scheme , see CELL v08.12.2017
    %% Fill CL{38~40} for cell type 1 and CL{41~44} for cell type 2 & 3
    for i=1:M
        CL=CELL{i};
        if CL{37}==1 % cell type 1
            cell1=CL{2};
...
%             A=[Cell1{5}-CL{5},Cell2{5}-CL{5},Cell3{5}-CL{5}]'/sqrt(CL{6});
            A=[Cell1{5}-CL{5},Cell2{5}-CL{5},Cell3{5}-CL{5}]'; % sqrt(CL{6}) is canceled out in in_cell_mapping as well
...
%                 A=[Coor1-CL{5},Coor2-CL{5},Coor3-CL{5}]'/sqrt(CL{6});
    ...
    for i=1:M
        ...
        for k=1:q1
            L_v1(:,k)=CL{5}+(-V1(:,k)*dt);
            if ~in_triangle(L_v1(:,k),CL{13},CL{14},CL{15})
                error('Please decrease the size of dt!');
            end
%             plot(CL{22},CL{23},CL{24},CL{25},CL{26},CL{27},'black','linewidth', 1);
%             hold on
%             Center=CL{5};
%             Advection_origin_point=L_v1(:,k);
%             plot(Center(1,1),Center(2,1),'Marker', 'o','Markersize',4, 'MarkerFaceColor', 'red','MarkerEdgeColor', 'red');
%             hold on
%             plot(Advection_origin_point(1,1),Advection_origin_point(2,1),'Marker', '*','Markersize',4, 'MarkerFaceColor', 'blue','MarkerEdgeColor', 'blue');
%             hold on
        end
%         if sum(CL{5}-L_v1(:,1))~=0
%             error('First advection origion sgould be located at the cell center!');
%         end
        CL{57}=L_v1;
        CELL{i}=CL;
    end
    ...
    for i=1:M
        ...
        for k=1:q2
            L_v2(:,k)=CL{5}+(-V2(:,k)*dt);
            if ~in_triangle(L_v2(:,k),CL{13},CL{14},CL{15})
                error('Please decrease the size of dt!');
            end
        end
        CL{58}=L_v2;
