...
switch FF
    case 0
        ...
        if strcmp(top,'Moving Wall')==1 && strcmp(right,'Periodic')==1 && strcmp(bottom,'Stationary Wall')==1 && strcmp(left,'Periodic')==1 % Couette flow, analytic solution is known
            ...
            f_nd=f_nd_old;
            f_nd_eq=f_nd;
        elseif (strcmp(top,'Stationary Wall')==1 && strcmp(bottom,'Stationary Wall')==1) % General Poiselle flow
            if strcmp(left,'Pressure Inlet')==1 && strcmp(right,'Pressure Outlet')==1  % Pressure driven flow
                Rho_l=Rho_in(4);
                Rho_r=Rho_out(2);
                a=(Rho_l-Rho_r)/(X1-X2);
                b=Rho_l-a*X1;
                ...
                f_old=zeros(qh,M);
                f_nd_old=zeros(qh,N);
                if  strcmp(left,'Velocity Inlet')==1 % Parabolic velocity profile
                    %%%Initial Condition of cell centriods
                    for r=1:M;
                        Cell=CELL{r};
                        coor=Cell{5};
...
%                 g_old(:,r)=T_bc(1)*f_old(:,r);
            ...
            g_old(:,r)=T_ana*f_old(:,r);
                ...
                g_old(:,r)=eqm_t(V2,[0;0],Rho_ref,T_bc(1),qt,wt,Rho_ref,FD);
            end
        end;
...
%             g_old(:,r)=T_ana*f_old(:,r);
    ...
    for r=1:N;
        ...
        if (Node{2}~=0 && Node{2}~=1) && Node{2}~=6
            if qh==qt
                bc_nd=Node{21};
                g_nd_old(:,r)=bc_nd(4,1)*f_nd_old(:,r);
            else
                bc_node=Node{21};
