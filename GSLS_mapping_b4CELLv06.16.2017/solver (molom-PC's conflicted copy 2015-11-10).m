...
for i=1:10
    if FT==0 % Explicit Euler
        if FS==0
            ...
            FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_r,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FTVD,FD,FM,FMP);
...
%                 f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/2/P{6}/W*(3*FC_old(:,r)-FC_old_old(:,r));
    ...
    elseif FT==4 % RK2
        ...
        %%%% Solving governing equation
        %%%% Calculate flux integral for all triangles
        FLX_K1=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd_K1,f_K1,f_eq_K1,Rho_K1,Rho_r,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FTVD,FD,FM,FMP);
        %%%% Calculate collision integral over triangles
        for r=1:M
            P=CELL{r};
...
%         end
    elseif FT==5 % RK4
        ...
        %%%% Solving governing equation
        %%%% Calculate flux integral for all triangles
        FLX_K1=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_r,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FTVD,FD,FM,FMP);
        %%%% Calculate collision integral over triangles
        for r=1:M
            P=CELL{r};
        ...
        %%%% Solving governing equation
        %%%% Calculate flux integral for all triangles
        FLX_K2=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_K1,f_eq_K1,Rho_K1,Rho_r,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FTVD,FD,FM,FMP);
        %%%% Calculate collision integral over triangles
        for r=1:M
            P=CELL{r};
        ...
        %%%% Solving governing equation
        %%%% Calculate flux integral for all triangles
        FLX_K3=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_K2,f_eq_K2,Rho_K2,Rho_r,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FTVD,FD,FM,FMP);
        %%%% Calculate collision integral over triangles
        for r=1:M
            P=CELL{r};
        ...
        %%%% Solving governing equation
        %%%% Calculate flux integral for all triangles
        FLX_K4=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_K3,f_eq_K3,Rho_K3,Rho_r,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FTVD,FD,FM,FMP);
        %%%% Calculate collision integral over triangles
        for r=1:M
            P=CELL{r};
...
%%% Adding the boundary values in the matrices
U_plt=U;
Rho_plt=Rho;
T_plt=T;
for r=1:N-N_I;
