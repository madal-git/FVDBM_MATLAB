function grd = gradient_tri(q,M,CELL,x_cell,x_node,fpdc,V)
...
%% This scheme inconsistent error on the non-periodic boundaries
for k=1:M
    CL=CELL{k};
    B=zeros(2,q);
    A=0;
    ...
    elseif CL{37}==3
        %% Old scheme, use third point by using boundary nodes
        cell_neigh=CL{44};
        nd=CL{45};
        w=CL{46};
        ...
        A=CL{50};
        if cell_neigh(1)==0
%             f_neigh=[(x_node(:,nd(1))+x_node(:,nd(2)))/2,x_cell(:,cell_neigh(2)),x_cell(:,cell_neigh(3))]; % Third point is the mid-point of the boundary face
%             f_neigh=[x_node(:,nd(1))+x_node(:,nd(2))-x_cell(:,k),x_cell(:,cell_neigh(2)),x_cell(:,cell_neigh(3))]; % Third point is the centroid of ghost cell
            fs_intcpt=(1-nd(3))*x_node(:,nd(1))+nd(3)*x_node(:,nd(2));
            f_neigh=[2*fs_intcpt-x_cell(:,k),x_cell(:,cell_neigh(2)),x_cell(:,cell_neigh(3))]; % Third point is the centroid of ghost cell
        elseif cell_neigh(2)==0
%             f_neigh=[x_cell(:,cell_neigh(1)),(x_node(:,nd(1))+x_node(:,nd(2)))/2,x_cell(:,cell_neigh(3))]; % Third point is the mid-point of the boundary face
%             f_neigh=[x_cell(:,cell_neigh(1)),x_node(:,nd(1))+x_node(:,nd(2))-x_cell(:,k),x_cell(:,cell_neigh(3))]; % Third point is the centroid of ghost cell
            fs_intcpt=(1-nd(3))*x_node(:,nd(1))+nd(3)*x_node(:,nd(2));
            f_neigh=[x_cell(:,cell_neigh(1)),2*fs_intcpt-x_cell(:,k),x_cell(:,cell_neigh(3))]; % Third point is the centroid of ghost cell
        elseif cell_neigh(3)==0
%             f_neigh=[x_cell(:,cell_neigh(1)),x_cell(:,cell_neigh(2)),(x_node(:,nd(1))+x_node(:,nd(2)))/2]; % Third point is the mid-point of the boundary face
%             f_neigh=[x_cell(:,cell_neigh(1)),x_cell(:,cell_neigh(2)),x_node(:,nd(1))+x_node(:,nd(2))-x_cell(:,k)]; % Third point is the centroid of ghost cell
            fs_intcpt=(1-nd(3))*x_node(:,nd(1))+nd(3)*x_node(:,nd(2));
            f_neigh=[x_cell(:,cell_neigh(1)),x_cell(:,cell_neigh(2)),2*fs_intcpt-x_cell(:,k)]; % Third point is the centroid of ghost cell
        else
            error('logic error!');
        end
        for i=1:q
        ...
        %% New scheme, only use existing centroid, don't use boundary nodes at all
%         cell_neigh=CL{44};
%         w=CL{46};
%         dis_vec=[CL{47},CL{48}];
%         A=CL{50};
%         if cell_neigh(1)==0
%             f_neigh=[x_cell(:,cell_neigh(2)),x_cell(:,cell_neigh(3))];
%         elseif cell_neigh(2)==0
%             f_neigh=[x_cell(:,cell_neigh(1)),x_cell(:,cell_neigh(3))];
%         elseif cell_neigh(3)==0
%             f_neigh=[x_cell(:,cell_neigh(1)),x_cell(:,cell_neigh(2))];
%         else
%             error('logic error!');
%         end
%         for i=1:q
...
%% Algorithm 4 see paper Maik Stiebler, Jonas Tolke, Manfred Krafczyk. An upwind discretization scheme for the finite volume lattice. Computers & Fluids
%% Such a scheme is only applied to cell type 1. For cell type 2 and 3, still use the first-order interpolation scheme
% for k=1:M
%     CL=CELL{k};
%     B=zeros(2,q);
%     if CL{37}==1 % Cell type 1
