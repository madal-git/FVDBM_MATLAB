...
Rho_in=[1.9,2,2,2]; % Inlet density [top,right,bottom,left]
F_rho_in=[0,0,0,0]; % Variable distribution type for inlet density [top,right,bottom,left]; 0 --- constant; 1 --- Linear; 2 --- Parabolic

Rho_out=[2,2,2,2]; % Outlet density [top,right,bottom,left]
F_rho_out=[0,0,0,0]; % Variable distribution type for outlet density [top,right,bottom,left]; 0 --- constant; 1 --- Linear; 2 --- Parabolic

U_r=[0,0,0,0;0,0,0,0]; % Reference velocity
U_m=[0.01/sqrt(3),0,0.01,0;0,0.1,0,0.2]; % Moving wall velocity [top,right,bottom,left]

U_in=[0,0.01,0.01,0.0;2,0,0,0]; % Inlet velocity [top,right,bottom,left]
...
U_out=[0,0.01/sqrt(3),0.01,0.01;2,0,0,0]; % Outlet velocity [top,right,bottom,left]
...
for i=1:N
    ...
    if ND{2}~=0
        nd_bc=ND{21};
        coor=ND{3};
        nd_bc(1)=(coor(2,1)-Y1)*(Rho_out(1,3)-Rho_out(1,1))/(Y1-Y2)+Rho_out(1,3);
        ND{21}=nd_bc;
        NODE{i}=ND;
    end
end
...
NPdc=131;
NInt=2015; % 2010 for U inlet; 2100 for Rho inlet; 2099 for U and Rho Inlet
NOut=3100; % 3100 for extrapolated u and v; 3199 and 3099 for both density and velocity prescribed
NStw=410;
NMow=510;
NWdp=615;
NIof=800;
...
NC71=7110;
NC72=7210;
NC73=7399; % 7310 for only velocity presribed; 7399 for both density and velocity prescribed
NC74=7400;
NC75=7500;
NC76=7622;
NC77=7700;
...
% Body_force=[0;-4*(Tau/3)^2*Rho_ref/(abs(Rho_out(1,3)-Rho_out(1,1))*(Y2-Y1)^3)];
Body_force=[0;-0.00001];
T_ref=1/3;
