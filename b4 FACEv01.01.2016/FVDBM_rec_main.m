...
[NODE,FACE]=stencil(CELL,NODE,FACE,V1,V2,N_I,N_I_N,N_L,N_H,h,dx,dy,X,Y,X1,X2,Y1,Y2,FM);
...
U_m=[0.1,0,0.01,0;0,0.1,0,0.2]; % Moving wall velocity [top,right,bottom,left]

U_in=[0,0.01,0.01,0.01;2,0,0,0]; % Inlet velocity [top,right,bottom,left]
F_u_in=[0,0,0,2;0,0,0,0]; % Variable distribution type for inlet velocity [top,right,bottom,left]; 0 --- constant; 1 --- Linear; 2 --- Parabolic
...
% Thermal
T_bc=[1,1.1,1.2,1.3,1.5];  % Temperature [top,right,bottom,left,immersed]
...
FS=1;  %%%% Splitting flag, 1----Collision first; 2----Advection first; 3----Strang splitting with advection in the middle; 4----Strang splitting with collision in the middle;
...
% 4---QUICK
% 5---QUICKEST
FInr=4; % Flux flag for interior face
FPdc=4; % Flux flag for periodic outer boundary face
FInt=4; % Flux flag for intlet face
FOut=4; % Flux flag for outlet face
FStw=4; % Flux flag for stationary wall face
FMow=4; % Flux flag for moving wall face
FWdp=4; % Flux flag for well developed face
%% Other variables
