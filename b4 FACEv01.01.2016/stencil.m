    ...
    for l=1:O
        ...
        for i=1:length(fc16)
            if fc16(1,i)~=0
                ...
                C3=fc21(5:6,i);
                if fc19(1,i)==4
                    if double(e+dis(C1,fc17(:,i)))~=double(e) || (double(e+dis(C2,fc17(:,i)))~=double(e) || double(e+dis(C3,fc17(:,i)))~=double(e))
            ...
            for i=1:length(fc16)
                if fc16(1,i)==0
                    ...
                    elseif in_zone_three
                        ...
                        if on_edge_one && (on_edge_two && on_edge_three)
                            % Check
                            if double(e+dis(C_c,fc26(:,i)))~=double(e)
                                error('Logic error!');
                            end
                            fc28(1,i)=4;
                        elseif on_edge_one
    ...
    for l=1:O
        ...
        if FC{24}==2 || (FC{24}==3 || FC{24}==4)
            ...
            for i=1:length(fc25)
                ...
                elseif fc28(1,i)==4
                    if double(e1+dis(fc26(:,i),C_fix))>single(e1)
                        error('The stencil point should be at the centroid!');
                    else
                        break;
                    end
    ...
    if FM==0
        ...
        for l=1:O
            ...
            if FC{24}==2
                for i=1:length(fc25)
                    P=CELL{fc25(1,i)};
                    if double(e1+sum(P{5}-fc26(:,i)))~=double(e1)
            ...
            for i=1:length(fc31)
                if fc31(1,i)~=0
                    if fc35(1,i)==4
                        ...
                        if double(e1+sum(fc37(1:2,i)-fc37(3:4,i)))~=double(e1) || double(e1+sum(fc37(3:4,i)-fc37(5:6,i)))~=double(e1)
            ...
            for i=1:length(fc38)
                if fc38(1,i)~=0
                    if fc42(1,i)==4
                        ...
                        if double(e1+sum(fc44(1:2,i)-fc44(3:4,i)))~=double(e1) || double(e1+sum(fc44(3:4,i)-fc44(5:6,i)))~=double(e1)
...
for l=1:O
    ...
    for k=1:q1
        ...
        %% S{10~11}
        s10(k,1)=(L1+L2*(L1+L3)/(L3-L2))/(L1+L2);
        s11(k,1)=-L2/(L3-L2);
        
        %% S{12~14}
        s12(k,1)=L2*L3/(L1+L2)/(L1+L3);
        s13(k,1)=(1+L2/(L3-L2))*L1/(L1+L2);
        s14(k,1)=-L1*L2/(L1+L3)/(L3-L2);
    end
    S{9}=s9;
    S{10}=s10;
    S{11}=s11;
