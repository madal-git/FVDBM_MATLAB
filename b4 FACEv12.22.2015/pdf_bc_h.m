function f_bc=pdf_bc_h(N_L,N_H,N_I,tau,dt,NODE,ND,f,feq,f_node,U_node,U,RHO_node,RHO,V,V1,V2,Rho_r,Rho_in,Rho_out,qh,wc,wh,fd,fdcy,NPdc,NInt,NOut,NStw,NMow,NWdp,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77)
...
elseif abs(ND{2})==31 % Pressure Outlet
    ...
    elseif NOut==3199 % v fixed to zero & extrapolated u + local average f_neq
        U_ex=node_star(ND,U,0);
        f_bc_eq(:,1)=eqm_h(V,[U_ex(1);0],Rho_bc,q,wh,Rho_r,fd);
        f_bc_neq(:,1)=node_star(ND,f,0)-node_star(ND,feq,0);
        f_bc=f_bc_eq+f_bc_neq;
    end
