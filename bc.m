...
% C{10, 11 or 12}=8          Inflow or Outflow
...
% C{19, 20 or 21}=8          Inflow or Outflow
    ...
    elseif NP{2}>0 %Outer boundary nodes
        ...
        if l<=N_I+N_L-2 %%%% Top
            ...
            elseif strcmp(top,'Fully Developed')==1
                NP{2}=6;
            elseif strcmp(top,'Inflow/Outflow')==1
                NP{2}=8;
            else
        ...
        elseif l>N_I+N_L-1 && l<=N_I+N_L-1+N_H-2 %%%% Right
            ...
            elseif strcmp(right,'Fully Developed')==1
                NP{2}=6;
            elseif strcmp(right,'Inflow/Outflow')==1
                NP{2}=8;
            else
                error('Please check spelling and input the correct boundary type!');
            end
        elseif l>N_I+N_L-1+N_H-1 && l<=N_I+N_L-1+N_H-2+N_L-1 %%%% Bottom
            ...
            elseif strcmp(bottom,'Fully Developed')==1
                NP{2}=6;
            elseif strcmp(bottom,'Inflow/Outflow')==1
                NP{2}=8;
            else
                error('Please check spelling and input the correct boundary type!');
            end
        elseif  l>N_I+N_L-1+N_H-2+N_L && l<N  %%%% Left
            ...
            elseif strcmp(left,'Fully Developed')==1
                NP{2}=6;
            elseif strcmp(left,'Inflow/Outflow')==1
                NP{2}=8;
            else
                error('Please check spelling and input the correct boundary type!');
            end
...
%         end
    elseif NP{2}<0 %Immersed boundary nodes
        ...
        elseif strcmp(immersed,'Inflow/Outflow')==1
        ...
        if l==N_I+N_L-1 || l==N_I+N_L-1+N_H-1 || l==N_I+N_L-1+N_H-2+N_L || l==N %%%% Top right corner, Bottom right  corner, Bottom left  corner, Top left corner. Can be replaced with if NP{2}>=0
            ...
            elseif abs(NU{2})==1 || abs(ND{2})==1
                ...
                elseif (abs(NU{2})==21 || abs(NU{2})==31) || (abs(ND{2})==21 || abs(ND{2})==31)
                    ...
                    NP{2}=(NP{2}/TEMP)*72;
                elseif abs(NU{2})==8 || abs(ND{2})==8
                    % One side is periodic, the other side is inflow/outflow
                    NP{2}=(NP{2}/TEMP)*78;
                else
                    error('Some corner nodes boardering periodic BC are absent from BC definition!')
                end
            elseif abs(NU{2})==8 || abs(ND{2})==8
                if abs(NU{2})==8 && abs(ND{2})==8
                    NP{2}=(NP{2}/TEMP)*80; % Both sides are inflow or outflow
                else
                    NP{2}=(NP{2}/TEMP)*79; % Only one side is inflow or outflow, the other side is either velocity or density
                end
            elseif (abs(NU{2})==4 || abs(NU{2})==5 || abs(NU{2})==20 || abs(NU{2})==30) && (abs(ND{2})==4 || abs(ND{2})==5 || abs(ND{2})==20 || abs(ND{2})==30)
...
for l=1:N
    ...
    if (abs(NP{2})/70)>=1 % All corner nodes, including immersed corner nodes
        ...
        elseif abs(NP{2})==77 % Fully Developed + Fully Developed
            ;
        elseif abs(NP{2})==78 % Periodic + Inflow/Outflow
            ;
        elseif abs(NP{2})==79 % Velocity or Density + Inflow/Outflow
            ;
        elseif abs(NP{2})==80 % Inflow/Outflow + Inflow/Outflow
            ;
        else
...
%% Find the boundary normal stencil information on all boundary nodes, ND{22}~ND{25}, see NODE v02.06.2019, the following scheme is proved to be not working
% for l=1:N
%     ND=NODE{l};
%     if ND{2}~=0 % Boundary nodes
%         %% ND{22}
%         %% Calculate the unit normal
%         NS=ND{18};
%         N_up=NODE{NS(1)}; % Upstream boundary node
%         N_down=NODE{NS(2)}; % Downstream boundary node
%         n_bc=norm_orthog(N_up{3},N_down{3});
%         %% Determine whether it is pointing inward, if not, flip the sign
%         num_star_cell=ND{4};
%         star_cell_index=ND{5};
%         distance=0;
%         for i=1:num_star_cell
%             CL=CELL{star_cell_index(i)};
%             distance=distance+CL{6};
%         end
%         distance=sqrt(2*distance/num_star_cell)/4;
%         S_test=ND{3}+n_bc*distance;
%         is_in_triangle=zeros(1,num_star_cell);
%         for i=1:num_star_cell
%             CL=CELL{star_cell_index(i)};
%             is_in_triangle(i)=in_triangle(S_test,CL{13},CL{14},CL{15});
%         end
%         
%         if sum(is_in_triangle)==1
%             ND{22}=n_bc;
%         elseif sum(is_in_triangle)==0
%             ND{22}=-n_bc;
%         else
%             error('Logic Error!');
%         end
%         %% The first entry in ND{23} and the first entry in ND{24}
%         cell_index=zeros(1,2);
%         Coord=zeros(2,2);
%         dist=zeros(1,num_star_cell);
%         for i=1:num_star_cell
%             CL=CELL{star_cell_index(i)};
%             dist(i)=dis(CL{5},norm_joint(ND{3},ND{22},CL{5}));
%         end
%         dist_min=min(dist);
%         for i=1:num_star_cell
%             if single(distance*10+dist(i))==single(distance*10+dist_min)
%                 break;
%             end
%         end
%         if i==num_star_cell
...
%         end
%         
%         cell_index(1,1)=star_cell_index(i);
%         CL_near=CELL{cell_index(1,1)};
%         Coord(:,1)=norm_joint(ND{3},ND{22},CL_near{5});
%         %% The second entry in ND{23} and the second entry in ND{24}
%         %% Determine the cell pool in which the search for further stencil point will be conducted
%         if mod(num_star_cell,2)==0 % the current boundary node is connected even number of cells
%             num_star_face=ND{12};
%             star_face=ND{13};
%             if num_star_face~=(num_star_cell+1)
%                 error('Logic Error!');
%             end
%             %% Find the middle face among star faces
%             for i=1:num_star_face
%                 FC=FACE{star_face(i)};
%                 dist(i)=dis(FC{7},norm_joint(ND{3},ND{22},FC{7}));
%             end
%             dist_min=min(dist);
%             for i=1:num_star_face
%                 if single(distance*10+dist(i))==single(distance*10+dist_min)
...
%                 end
%             end
%             face_found=star_face(i);
%             FC_found=FACE{face_found};
%             %% Find the target node
%             nd1=FC_found{8};
%             nd2=FC_found{9};
%             nd_found=setxor([nd1,nd2],ND{1});
%             if length(nd_found)~=1
%                 error('Logic Error!');
%             end
%             ND_found=NODE{nd_found};
%             %% Find the cell pool
%             pool_cell=setxor(ND_found{5},intersect(ND_found{5},ND{5}));
%             num_pool_cell=length(pool_cell);
%             if num_pool_cell~=(ND_found{4}-2)
%                 error('Logic Error!');
%             end
%         else % the current boundary node is connected even number of cells
%             %% find the other two nodes other than the current boundary node that belongs to the cell that contains the near stencil point
%             three_node_near_cell=[CL_near{7},CL_near{8},CL_near{9}];
%             two_node_near_cell=setxor(three_node_near_cell,ND{1});
%             if length(two_node_near_cell)~=2
%                 error('Logic Error!');
%             end
%             %% find the cell that is the neighbor of the cell that contains the near stencil point
%             ND1=NODE{two_node_near_cell(1)};
%             ND2=NODE{two_node_near_cell(2)};
%             face_star_1=ND1{13};
%             face_star_2=ND2{13};
%             shared_face=intersect(face_star_1,face_star_2);
%             if length(shared_face)~=1
%                 error('Logic Error!');
%             end
%             FC=FACE{shared_face};
%             fc12=FC{12};
...
%             fc_neigh_1=fc12(1);
%             fc_neigh_2=fc13(1);
%             cell_neigh=setxor([fc_neigh_1,fc_neigh_2],CL_near{1});
%             if length(cell_neigh)~=1
%                 error('Logic Error!');
%             end
%             %% Find the node that belongs to the neighbor cell and is apposing the found shared face
%             CL_neigh=CELL{cell_neigh};
%             three_node_cell_neigh=[CL_neigh{7},CL_neigh{8},CL_neigh{9}];
%             node_target=setxor(three_node_cell_neigh,two_node_near_cell);
%             if length(node_target)~=1
%                 error('Logic Error!');
%             end
%             %% Find the cell pool
%             ND_found=NODE{node_target};
%             num_pool_cell=ND_found{4};
%             pool_cell=ND_found{5};
%         end
%         %% Calculate the coordinate of the foot of the centroid of each cell in the pool on the stencil line, calculate the distance between the foot and the current boundary node, and determine whether the foot is in its own CV
%         coord_foot=zeros(2,num_pool_cell);
%         dist=zeros(1,num_pool_cell);
%         is_in_cv=zeros(1,num_pool_cell);
%         for i=1:num_pool_cell
%             CL=CELL{pool_cell(i)};
%             coord_foot(:,i)=norm_joint(ND{3},ND{22},CL{5});
%             dist(i)=dis(ND{3},coord_foot(:,i));
%             is_in_cv(i)=in_triangle(coord_foot(:,i),CL{13},CL{14},CL{15});
%         end
%         %% locate the cell that satisfy both conditions (1. the foot is in its own CV; 2. the distance to the boundary node is shortest)
%         if sum(is_in_cv)==0 % No cell is found, this will most likely happen to IRT mesh
%             %error('Not successful to find the further stencil point for the current boundary node!');
%             dist_max=max(dist);
%             for i=1:num_pool_cell
%                 if single(distance*10+dist(i))==single(distance*10+dist_max)
%                     break;
%                 end
%             end
%             cell_index(1,2)=pool_cell(i);
%             Coord(:,2)=coord_foot(:,i);
%         elseif sum(is_in_cv)==1 % The only found cell
%             for i=1:num_pool_cell
%                 if is_in_cv(i)==1
%                     break;
%                 end
...
%             cell_index(1,2)=pool_cell(i);
%             Coord(:,2)=coord_foot(:,i);
%         else % More than one found cells
%             %% Sample the cells that satisfy condition 1 (the foot is in its own CV)
%             num_pool_cell_sampled=sum(is_in_cv);
%             Coord_sampled=0;
%             pool_cell_sampled=0;
%             for i=1:num_pool_cell
%                 if is_in_cv(i)==1
%                     if pool_cell_sampled==0
%                         pool_cell_sampled=pool_cell(i);
%                         Coord_sampled=coord_foot(:,i);
%                     else
%                         pool_cell_sampled(end+1)=pool_cell(i);
%                         Coord_sampled(:,end+1)=coord_foot(:,i);
%                     end
%                 end
%             end
%             % Check
%             if length(pool_cell_sampled)~=num_pool_cell_sampled || length(Coord_sampled)~=num_pool_cell_sampled
%                 error('Logic Error!');
%             end
%             %% Calculate the min distance for each sampled foot to the current boundary node
%             dist_sampled=zeros(1,num_pool_cell_sampled);
%             for i=1:num_pool_cell_sampled
%                 dist_sampled(i)=dis(Coord_sampled(:,i),ND{3});
%             end
%             dist_sampled_min=min(dist_sampled);
%             %% Find the cell whose foot is closest to the current boundary node
%             for i=1:num_pool_cell_sampled
%                 if single(distance*10+dist_sampled(i))==single(distance*10+dist_sampled_min)
%                     break;
%                 end
%             end
%             if i==num_pool_cell_sampled
%                 if single(distance*10+dist_sampled(i))~=single(distance*10+dist_sampled_min)
%                     error('Logic Error!');
%                 end
%             end
%             cell_index(1,2)=pool_cell_sampled(i);
%             Coord(:,2)=Coord_sampled(:,i);
%             % Final check
%             CL_further=CELL{cell_index(1,2)};
%             if ~in_triangle(Coord(:,2),CL_further{13},CL_further{14},CL_further{15})
%                 error('The search for further stencil point failed!');
%             end
%         end
%         ND{23}=cell_index;
%         ND{24}=Coord;
%         ND{25}=[1+dis(ND{3},Coord(:,1))/dis(Coord(:,2),Coord(:,1)),-dis(ND{3},Coord(:,1))/dis(Coord(:,2),Coord(:,1))];
%     end
%     NODE{l}=ND;
% end

% %% Find the boundary normal stencil information on all boundary nodes, ND{22}~ND{23}, in order to use second-order mapping (FMP=7) on the boundary node, see NODE v04.11.2019, the following scheme is proved to be not working
% for l=1:N
%     ND=NODE{l};
%     if ND{2}~=0 % Boundary nodes
%         %% ND{22}
...
%         N_down=NODE{NS(2)}; % Downstream boundary node
%         n_bc=norm_orthog(N_up{3},N_down{3});
%         % Determine whether the current node is a corner node
%         num_star_cell=ND{4};
%         star_cell_index=ND{5};
%         if num_star_cell<=2 % Corner node
%             ND{22}=0; % Use node_start
%             ND{23}=0;
%         else % Other nodes, including inner and outer boundaries
%             ND{22}=1; % Use in_cell_mapping with FMP=7
%             % Determine whether it is pointing inward, if not, flip the sign
%             distance=0;
%             for i=1:num_star_cell
%                 CL=CELL{star_cell_index(i)};
...
%             for i=1:num_star_cell
%                 CL=CELL{star_cell_index(i)};
%                 dist(i)=dis(CL{5},norm_joint(ND{3},n_bc,CL{5}));
%             end
%             dist_min=min(dist);
%             for i=1:num_star_cell
%                 if single(distance*10+dist(i))==single(distance*10+dist_min)
...
%% Find the boundary normal stencil information on all boundary nodes, ND{22}~ND{23}, in order to use second-order mapping on the boundary node, see NODE v04.12.2019
for l=1:N
    ND=NODE{l};
    if ND{2}~=0 % Boundary nodes
        % Determine whether the current node has less than two connected
        % triangles
        num_star_cell=ND{4};
        star_cell_index=ND{5};
        if num_star_cell<=2 % Corner node
            ...
            ND{22}=1; % Use in_cell_mapping with FMP=7
            X=zeros(num_star_cell,3);
            for i=1:num_star_cell
                CL=CELL{star_cell_index(i)};
                coord_cell=CL{5};
                X(i,:)=[1, coord_cell(1,1), coord_cell(2,1)];
            end
            ND{23}=inv(X'*X)*X';
        end
    end
