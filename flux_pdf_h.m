function [fl,pdf_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,feq,pdf_face_center_old,RHO,Rho_r,V,dt,wl,wlb,wh,V1,V2,interior,periodic,inlet,outlet,s_wall,m_wall,well_dp,iof,fupd,ftvd,fpdc,fd,fm,fmp,fe)
% [fl,pdf_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,feq,pdf_face_center_old,RHO,Rho_r,V,dt,wl,wlb,wh,V1,V2,interior,periodic,inlet,outlet,s_wall,m_wall,well_dp,iof,fupd,ftvd,fpdc,fd,fm,fmp,fe)
% calculates the total flux of pdf occured to each trianglar cell and the pdf value at the face center of current instance.
...
% pdf_face_center_old is the pdf at the face center at previous time step
% for the entire FACE
...
% FMP=6---least-square
% FMp=7---Plane Fitting scheme (Xiaofeng)
...
% pdf_face_center_new is the pdf at the face center of the entire FACE for
...
%     %% Calculate the pdf at face center for Lagrangian advection with 2nd-order mapping
%     if fupd~=0
%         if FC{24}==2 || FC{24}==3
%             coord_lagrangian=ls_23{30};
%         elseif FC{24}==1 || FC{24}==4
%             coord_lagrangian=ls_14{30};
%         else
%             error('Logic Error!');
%         end
%         
%         for j=1:q
%             if FC{2}==0
%                 if latt_dir_marker(j)==1
%                     cl=FC{12};
%                     in_this_cell=cl(1);
%                 elseif latt_dir_marker(j)==0
%                     cl=FC{13};
%                     in_this_cell=cl(1);
%                 else
%                     error('Logic Error');
...
%                 end
%             end
%             pdf_face_center_new(j,r)=in_cell_mapping('dummy',CELL,'dummy',f_old(j,:),f_nd(j,:),in_this_cell,coord_lagrangian(:,j),'dummy','dummy',gf,7,V,fpdc);
%         end
%     end
    ...
    if face_bc_flag==0 % Interior face
        ...
        if interior<=1 % 1st-order upwind and LW
            if face_type==1
                ...
                fs_d=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_1(:,2),s_stcl_coord_1(:,2),s_map_id_1(:,2),s_map_coord_1(:,2),gf,fmp,V,fpdc);
                fs_u=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_1(:,3),s_stcl_coord_1(:,3),s_map_id_1(:,3),s_map_coord_1(:,3),gf,fmp,V,fpdc);
            ...
            elseif face_type==3 || face_type==4
                %% Create the pdf values at the stencil points
                fs_d=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_234(:,2),s_stcl_coord_234(:,2),s_map_id_234(:,2),s_map_coord_234(:,2),gf,fmp,V,fpdc);
                fs_u=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_234(:,3),s_stcl_coord_234(:,3),s_map_id_234(:,3),s_map_coord_234(:,3),gf,fmp,V,fpdc);
                %% Create the pdf values at the centroids along the stencil
...
%                     fs_dd=inter_extrp(0,dis(Coor_intcp,s_stcl_coord_1(:,2)),dis(Coor_intcp,s_stcl_coord_1(:,3)),fs_intcpt,fs_d,fs_u,-dis(Coor_intcp,s_stcl_coord_1(:,1)),fe);
                    ...
                    %% Create the pdf values at the stencil points
                    fs_dd=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_1(:,1),s_stcl_coord_1(:,1),s_map_id_1(:,1),s_map_coord_1(:,1),gf,fmp,V,fpdc);
                    %% Create the pdf values at the centroids along the stencil
...
%                     fs_uu=inter_extrp(0,dis(Coor_intcp,s_stcl_coord_1(:,3)),dis(Coor_intcp,s_stcl_coord_1(:,2)),fs_intcpt,fs_u,fs_d,-dis(Coor_intcp,s_stcl_coord_1(:,4)),fe);
                    ...
                    %% Create the pdf values at the stencil points
                    fs_uu=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_1(:,4),s_stcl_coord_1(:,4),s_map_id_1(:,4),s_map_coord_1(:,4),gf,fmp,V,fpdc);
                    %% Create the pdf values at the centroids along the stencil
...
%                     fs_dd=inter_extrp(0,dis(Coor_intcp,s_stcl_coord_234(:,2)),dis(Coor_intcp,s_stcl_coord_234(:,3)),fs_intcpt,fs_d,fs_u,-dis(Coor_intcp,s_stcl_coord_234(:,1)),fe);
                    ...
                    %% Create the pdf values at the stencil points
                    fs_dd=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_234(:,1),s_stcl_coord_234(:,1),s_map_id_234(:,1),s_map_coord_234(:,1),gf,fmp,V,fpdc);
                    %% Create the pdf values at the centroids along the stencil
...
%                     fs_uu=inter_extrp(0,dis(Coor_intcp,s_stcl_coord_234(:,3)),dis(Coor_intcp,s_stcl_coord_234(:,2)),fs_intcpt,fs_u,fs_d,-dis(Coor_intcp,s_stcl_coord_234(:,4)),fe);
                    ...
                    %% Create the pdf values at the stencil points
                    fs_uu=in_cell_mapping(FC,CELL,NODE,f_old,f_nd,s_cell_234(:,4),s_stcl_coord_234(:,4),s_map_id_234(:,4),s_map_coord_234(:,4),gf,fmp,V,fpdc);
                    %% Create the pdf values at the centroids along the stencil
        ...
        if face_type==1 || face_type==4
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_14,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,interior,fupd,ftvd,fpdc);
        elseif face_type==3
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,interior,fupd,ftvd,fpdc);
        else
            error('Logic error!');
        end
    elseif face_bc_flag>0 % Outer boundary face
        if face_bc_flag==1 % Periodic
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,periodic,fupd,ftvd,fpdc);
...
%                     fs_uu=fs_intcpt; % A even better scheme
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,inlet,fupd,ftvd,fpdc);
...
%                     fs_uu=fs_intcpt; % A even better scheme
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,outlet,fupd,ftvd,fpdc);
...
%                     fs_uu=fs_intcpt; % A even better scheme
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,s_wall,fupd,ftvd,fpdc);
...
%                     fs_uu=fs_intcpt; % A even better scheme
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,m_wall,fupd,ftvd,fpdc);
        elseif face_bc_flag==6
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,well_dp,fupd,ftvd,fpdc);
...
%                     fs_c=(fs_d*s_dis(1,1)-fs_dd*s_dis(1,2))/(s_dis(1,1)-s_dis(1,2)); % Explode
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_23,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,iof,fupd,ftvd,fpdc);
        else
...
%                     fs_uu=fs_intcpt; % A even better scheme
            ...
            %% calculate the PDFs at the face center
            [f_face,pdf_face_center_new(:,r)]=pdf_face(fs_dd,fs_d,fs_u,fs_uu,fs_dd_ctd,fs_d_ctd,fs_u_ctd,fs_uu_ctd,pdf_face_center_old(:,r),ls_14,latt_dir_marker,dt,face_normal_lattice,f_old,f_nd,FC,CELL,s_wall,fupd,ftvd,fpdc);
        else
            error('Wrong flag for face on outer boundaries!');
        end
    else
...
%     fl_face(:,r)=f_face*FC{3}; % This explodes
