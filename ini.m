    ...
    case 0
        ...
        elseif (strcmp(top,'Stationary Wall')==1 && strcmp(bottom,'Stationary Wall')==1) && (strcmp(left,'Velocity Inlet')==1) % Velocity driven Poiselle flow
                ...
                f_nd_old=zeros(qh,N);
                    ...
                    f_nd=f_nd_old;
                    f_nd_eq=f_nd;
        elseif (strcmp(top,'Moving Wall')==1 && strcmp(bottom,'Stationary Wall')==1) && (strcmp(left,'Stationary Wall')==1 && strcmp(right,'Stationary Wall')==1) % lid-driven cavity
            for r=1:M
                f_old(1:qh,r)=eqm_h(V,[0;0],Rho_ref,1,qh,wh,Rho_ref,FD); % Pertubation
            end
            f_old_old=f_old;
            f_eq=f_old;
            %%%%Initial Condition of nodes
...
%             end
        ...
        elseif strcmp(top,'Fully Developed')==1 && strcmp(right,'Stationary Wall')==1 && strcmp(bottom,'Fully Developed')==1 && strcmp(left,'Stationary Wall')==1 % 1D conduction
            ...
            for r=1:M
                %%% Calculate PDF for hydrodynamics
                f_old(1:qh,r)=eqm_h(V,[0;0],Rho_ref,T_ref,qh,wh,Rho_ref,FD);
            end
            f_old_old=f_old;
            %%%%Initial Condition of nodes
            for r=1:N
                ...
                bc_node=Node{21};
                if Node{2}~=0
                    f_nd_old(1:qh,r)=eqm_h(V,[0;0],Rho_ref,bc_node(4),qh,wh,Rho_ref,FD);
                else
                    f_nd_old(:,r)=node_star(Node,f_old,0);
                end
            end
            f_nd=f_nd_old;
            f_nd_eq=f_nd;
        elseif strcmp(top,'Stationary Wall')==1 && strcmp(right,'Fully Developed')==1 && strcmp(bottom,'Fully Developed')==1 && strcmp(left,'Stationary Wall')==1 && strcmp(immersed,'Stationary Wall')==1% Circ_hole conduction
            %%%Initial Condition of cell centriods
            for r=1:M
                %%% Calculate PDF for hydrodynamics
                f_old(1:qh,r)=eqm_h(V,[0;0],Rho_ref,T_ref,qh,wh,Rho_ref,FD);
...
%                 f_old(1:qh,r)=eqm_h(V,(rand(2,1)-0.5)/100,Rho_ref,1,qh,wh,Rho_ref,FD); % Pertubation
                % uniform inlet velovity
%                 f_old(1:qh,r)=eqm_h(V,U_in(:,4),Rho_ref,1,qh,wh,Rho_ref,FD);
                f_old(1:qh,r)=eqm_h(V,[0;0],Rho_ref,1,qh,wh,Rho_ref,FD);
            end
            f_old_old=f_old;
            f_eq=f_old;
    ...
    elseif strcmp(top,'Fully Developed')==1 && strcmp(right,'Stationary Wall')==1 && strcmp(bottom,'Fully Developed')==1 && strcmp(left,'Stationary Wall')==1 % 1D conduction
        ...
        if FCD==1 %% Pure conduction
            for r=1:M
                Cell=CELL{r};
                coor=Cell{5};
                T(1,r)=T_bc(4)+(T_bc(4)-T_bc(2))/(X1-X2)*(coor(1,1)-X1);
            end
            for r=1:M
                g_old(:,r)=eqm_t(V2,U(:,r),Rho_ref,T(1,r),qt,wt,Rho_ref,FD);
            end
    ...
    elseif strcmp(top,'Stationary Wall')==1 && strcmp(right,'Fully Developed')==1 && strcmp(bottom,'Fully Developed')==1 && strcmp(left,'Stationary Wall')==1 && strcmp(immersed,'Stationary Wall')==1% Circ_hole conduction
        ...
        if FCD==1 %% Pure conduction
            for r=1:M
                Cell=CELL{r};
                coor=Cell{5};
                if dis(coor,[(X2+X1)/2;(Y2+Y1)/2])<=0.4
                    T(1,r)=T_bc(5);
                else
                    T_max=T_bc(5)/exp(-((X2-X1)/5)^2/((X2-X1)/2)^2);
                    T(1,r)=T_max*exp(-((coor(1,1)-(X2-X1)/2)^2+(coor(2,1)-(Y2-Y1)/2)^2)/((X2-X1)/4+(Y2-Y1)/4)^2);
                end
            end
            for r=1:M
                g_old(:,r)=eqm_t(V2,[0;0],Rho_ref,T(1,r),qt,wt,Rho_ref,FD);
            end
        else
            error('Other type of flows are temporarily not available for thermal model');
        end
...
%     end
    for l=1:N
        ...
        if NP{2}==0 % Interior nodes
            ...
            g_nd(:,l)=pdf_bc_t(N_L,N_H,N_I,X1,X2,Y1,Y2,Tau_t,dt,NODE,NP,f_old,f_eq,f_nd,g_old,g_eq,g_nd,U_nd,U,Rho_nd,Rho,T,V2,V1,V2,Rho_ref,Rho_in,Rho_out,qh,qt,wc,wh,wt,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
        end
    end
    T_nd=macro_t(g_nd,V2,Rho_nd);
end
%%% Calculate the velocity megnitude in cell centroid
xlin=linspace(X1,X2,300);
ylin=linspace(Y1,Y2,300);
...
%% Initialize the tracer plot
% if FTRC==1
%     trc=tracer;
%     trc.L_x=(X2-X1);
%     trc.L_y=(Y2-Y1);
%     trc.num_x=Num_tracer_x;
%     trc.num_y=Num_tracer_y;
%     trc.X1=X1;
%     trc.X2=X2;
%     trc.Y1=Y1;
%     trc.Y2=Y2;
...
%     trc.ini;
% end
% %% Initialize for other plot
% if FANI==1
%     pathname = 'G:\Dropbox\Pitt!\LBM\repos\fvdbm\anime'; % Change this for different platform
%     mkdir anime;
%     FIG_COUNTER=0;
% end