...
%% Plot f_eq^n+1-f_eq^n
TIME=TT(2:end);
for i=1:qh
    figure;
    plot(TIME,FEQ_Diff_DBM(i,:));
    title(['Time history of f^e^q^,^n^+^1-f^e^q^,^n in direction ', num2str(i-1)]);
    xlabel('Time Step');
    ylabel(['f^e^q^,^n^+^1_',num2str(i-1),'-','f^e^q^,^n_',num2str(i-1)]);
end
