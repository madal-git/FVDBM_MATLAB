function [Nu, Angle, X, Y, V_90, V_180, T_90, T_180]=plt_nc(CELL,NODE,M,N,X1,X2,Y1,Y2,U,U_nd,T,T_nd,T_bc,Body_force,Tau,Tau_t)
...
xlabel('X^*');
ylabel('V^*');
V_90=v*Diameter/Ra^0.5/(Tau_t/3);
...
xlabel('Y^*');
ylabel('U^*');
V_180=v_180*Diameter/Ra^0.25/(Tau_t/3);

% Plot temperature
figure(4)
hold on
plot(Y,(t_180-T_bc(1))/(T_bc(5)-T_bc(1)))
T_180=(t_180-T_bc(1))/(T_bc(5)-T_bc(1));