tic;
for i=1:t_stop
    if FHYD_solver_switch==1
        if FT==0 % Explicit Euler
            if FS==0
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        if W~=1 && FMP>1
                            %f_nd(:,l)=node_star(NP,f_old,0);
                        end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%% Nodal variable update
                % if W~=1
                %%%%%% Update macro velocity and density at nodes
                %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Calculate equilibrium pdf at nodes
                %     for r=1:N
                %         f_nd_eq(:,r)=eqm_h(V,U_nd(:,r),Rho_nd(1,r),T(1,r),qh,wh,Rho_ref,FD);
                %     end
                %         for l=1:N;
                %             NP=NODE{l};
                %             if NP{2}==0
                %                 f_nd_eq(:,l)=node_star(NP,f_eq,0);
                %             else
                %                 f_nd_eq(:,l)=eqm_h(V,U_nd(:,l),Rho_nd(1,l),T(1,r),qh,wh,Rho_ref,FD);
                %             end
                %         end
                % end
                %%%% Solving governing equation
                %%%% Calculate flux integral for all triangles
                if tt==1
                    pdf_h_face_center_old=zeros(qh,O);
                    for r=1:O
                        FC=FACE{r};
                        pdf_h_face_center_old(:,r)=(f_nd(:,FC{8})+f_nd(:,FC{9}))/2;
                    end
                else
                        pdf_h_face_center_old=pdf_h_face_center_new;
                end
                [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate collision integral over triangles
                for r=1:M
                    P=CELL{r};
                    if FMRT==0
                        FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
                    elseif FMRT==1
                        error('Temporarily unavailable!');
                    elseif FMRT==2
                        m_h(:,r)=M_h*f_old(:,r);
                        m_h_eq(:,r)=mrt_2d_eqm_moment (d,qh,Rho(1,r),U(:,r));
                        FCOL(:,r)=(M_h_inv*S_h)*(m_h_eq(:,r)-m_h(:,r))*P{6};
                    else
                        error('Wrong flag for collision model!');
                    end
                end
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                for r=1:M
                    P=CELL{r};
                    if FFC==0
                        f(:,r)=f_old(:,r)+dt/P{6}*(FCOL(:,r)-FLX(:,r));
                    %f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(FCOL(:,r)-FLX(:,r));
                    elseif FFC==1
                        FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                        f(:,r)=f_old(:,r)+dt/P{6}*(FCOL(:,r)+P{6}*FORCE(:,r)-FLX(:,r));
                    else
                        error('Wrong flag for force');
                    end
                end
            elseif FS==1 % Collision first
                %             %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %             %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                % Collision
                for r=1:M
                    f_old(:,r)=(f_old(:,r)-f_eq(:,r))*exp(-dt/Tau)+f_eq(:,r);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        if W~=1 && FMP>1
                            %f_nd(:,l)=node_star(NP,f_old,0);
                        end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%%% Update macro velocity and density at nodes
                %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %             %%%% Calculate equilibrium pdf at nodes
                %                 for r=1:N
                %                     f_nd_eq(:,r)=eqm_h(V,U_nd(:,r),Rho_nd(1,r),T(1,r),qh,wh,Rho_ref,FD);
                %                 end
                % %                     for l=1:N;
                % %                         NP=NODE{l};
                % %                         if NP{2}==0
                % %                             f_nd_eq(:,l)=node_star(NP,f_eq,0);
                % %                         else
                % %                             f_nd_eq(:,l)=eqm_h(V,U_nd(:,l),Rho_nd(1,l),T(1,r),qh,wh,Rho_ref,FD);
                % %                         end
                % %                     end
                %
                %             %%%% Solving governing equation
                %             %%%% Calculate flux integral for all triangles
                %             %%%% After collision pdf at cell centriod and vertices
                %             for r=1:M
                %                 f_old(:,r)=f_old(:,r)+(f_eq(:,r)-f_old(:,r))/Tau*dt;
                %             end
                %             for l=1:N;
                %                 f_nd(:,l)=f_nd(:,l)+(f_nd_eq(:,l)-f_nd(:,l))/Tau*dt;
                %             end
                
                %%%% Calculate flux integral for all triangles
                FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                for r=1:M
                    P=CELL{r};
                    f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(-FLX(:,r));
                end
            elseif FS==2 % Advection first
                %             %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %             %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        if W~=1 && FMP>1
                            %f_nd(:,l)=node_star(NP,f_old,0);
                        end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%%% Update macro velocity and density at nodes
                %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Calculate flux integral for all triangles
                FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                for r=1:M
                    P=CELL{r};
                    f_old(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(-FLX(:,r));
                end
                %             %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %             %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                % Collision
                for r=1:M
                    f(:,r)=(f_old(:,r)-f_eq(:,r))*exp(-dt/Tau)+f_eq(:,r);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
            elseif FS==3 % Collision first with new method for collision
                if tt==1
                    %             %%%%%% Update macro velocity and density at cell centroid
                    [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                    %             %%%% Calculate equilibrium pdf at cell centroids
                    for r=1:M
                        f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                    end
                    %%%%f_eq_old
                    f_eq_old=f_eq;
                    % Collision
                    for r=1:M
                        f_old(:,r)=(f_old(:,r)-f_eq(:,r))*exp(-dt/Tau)+f_eq(:,r);
                    end
                    %%%%%%Nodal pdf updated and boundary conditions applied
                    for l=1:N
                        NP=NODE{l};
                        if NP{2}==0 % Interior nodes
                            if W~=1 && FMP>1
                                %f_nd(:,l)=node_star(NP,f_old,0);
                            end
                        else % nodes on boundary
                            f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                        end
                    end
                    %%%%%% Update macro velocity and density at nodes
                    %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                    %%%% Calculate flux integral for all triangles
                    FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                    %%%% Calculate the current pdf at cell centroid/// See derivation in
                    %%%% notes
                    for r=1:M
                        P=CELL{r};
                        f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(-FLX(:,r));
                    end
                else
                    %             %%%%%% Update macro velocity and density at cell centroid
                    [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                    %             %%%% Calculate equilibrium pdf at cell centroids
                    for r=1:M
                        f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                    end
                    %%%%f_eq_old
                    f_eq_new=2*f_eq-f_eq_old;
                    % Collision
                    Grad_f_eq=(f_eq-f_eq_old)/dt;
                    for r=1:M
                        f_neq(:,r)=(f_old(:,r)-f_eq(:,r)-Grad_f_eq(:,r)*Tau)*exp(-dt/Tau)+Grad_f_eq(:,r)*Tau;
                    end
                    f_old=f_neq+f_eq_new;
                    %%%%%%Nodal pdf updated and boundary conditions applied
                    for l=1:N
                        NP=NODE{l};
                        if NP{2}==0 % Interior nodes
                            if W~=1 && FMP>1
                                %f_nd(:,l)=node_star(NP,f_old,0);
                            end
                        else % nodes on boundary
                            f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                        end
                    end
                    %%%%%% Update macro velocity and density at nodes
                    %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                    %%%% Calculate flux integral for all triangles
                    FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                    %%%% Calculate the current pdf at cell centroid/// See derivation in
                    %%%% notes
                    for r=1:M
                        P=CELL{r};
                        f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(-FLX(:,r));
                    end
                    %%%%f_eq_old
                    f_eq_old=f_eq;
                end
            else
                error('The flag for splitting option is invalid or unavailable!');
            end
        elseif FT==1 % Implicit Euler, CI/AE
            if tt==1
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                if FTH==1 && qh~=37
                    T=macro_t(g_old,V2,Rho);
                end
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        % if W~=1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                        % end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%% Nodal variable update
                % if W~=1
                %%%%%% Update macro velocity and density at nodes
                [Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%%%% Initiate the value of pdf_face_center_old
                pdf_h_face_center_old=zeros(qh,O);
                for r=1:O
                    FC=FACE{r};
                    pdf_h_face_center_old(:,r)=(f_nd(:,FC{8})+f_nd(:,FC{9}))/2;
                end
                %%%% Calculate flux integral for all triangles
                [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate collision integral over triangles
                for r=1:M
                    P=CELL{r};
                    FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
                end
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                if FFC==0
                    f=f_old+dt*(FCOL-FLX)./Vol;
                        %                 f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(FCOL(:,r)-FLX(:,r));
                elseif FFC==1
                    FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                    f=f_old+dt*((FCOL-FLX)./Vol+FORCE);
                else
                    error('Wrong flag for force term!');
                end
                %%%%f_eq_old
                f_eq_old=f_eq;
                f_old=f;
            else
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                if FTH==1 && qh~=37
                    T=macro_t(g_old,V2,Rho);
                end
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        %                     if W~=1 && FMP>1
                        %                         %f_nd(:,l)=node_star(NP,f_old,0);
                        %                     end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%% Nodal variable update
                % if W~=1
                %%%%%% Update macro velocity and density at nodes
                %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Calculate flux integral for all triangles
                pdf_h_face_center_old=pdf_h_face_center_new;
                [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %             if FInr==1 || FInr==9 % Godunov flux, requires different time marching
                %                 for r=1:M
                %                     P=CELL{r};
                %                     f(:,r)=(f_old(:,r)*(2*Tau-dt)+dt*(3*f_eq(:,r)-f_eq_old(:,r))-FLX(:,r)*2*Tau*dt/P{6})/(2*Tau+dt); % Scheme 1, linear interpolation of collision term, this proved to be the same as the IC-EA scheme
                % %                     f(:,r)=(1-exp(-dt/Tau))*f_eq(:,r)+exp(-dt/Tau)*f_old(:,r)-FLX(:,r)*dt/P{6}; % Scheme 2, exp decaying of collision term
                %                 end
                %             else
                %%%% Calculate the extrapolated f_eq_im
                f_eq_im=2*f_eq-f_eq_old;
                %%%% Calculate the current pdf at cell centroid-Implicit scheme
                if FFC==0
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        f=((1-dt/Tau/2)*f_old-FLX*dt./Vol+(f_eq_im+f_eq)*dt/Tau/2)/(1+dt/Tau/2);
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        f=(f_old-FLX*dt./Vol+f_eq_im*dt/Tau)/(1+dt/Tau);
                    end
                elseif FFC==1
                    FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        f=((1-dt/Tau/2)*f_old-FLX*dt./Vol+(f_eq_im+f_eq)*dt/Tau/2+FORCE*dt/2)/(1+dt/Tau/2);
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        f=(f_old-FLX*dt./Vol+f_eq_im*dt/Tau+dt*FORCE)/(1+dt/Tau);
                    end
                else
                    error('Wrong flag for force term!');
                end
                %             end
                %%%%f_eq_old
                f_eq_old=f_eq;
            end
        elseif FT==2 % Implicit Euler, CI/AI
            if tt==1
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        % if W~=1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                        % end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%% Nodal variable update
                % if W~=1
                %%%%%% Update macro velocity and density at nodes
                [Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Calculate flux integral for all triangles
                FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate collision integral over triangles
                for r=1:M
                    P=CELL{r};
                    FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
                end
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                for r=1:M
                    P=CELL{r};
                    %f(:,r)=f_old(:,r)+dt/P{6}*(FCOL(:,r)-FLX(:,r));
                    f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(FCOL(:,r)-FLX(:,r));
                end
                %%%%f_eq_old
                f_eq_old=f_eq;
                FLX_old=FLX;
            else
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        if W~=1 && FMP>1
                            %f_nd(:,l)=node_star(NP,f_old,0);
                        end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%% Nodal variable update
                % if W~=1
                %%%%%% Update macro velocity and density at nodes
                %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Calculate flux integral for all triangles
                FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate the extrapolated f_eq_im and FLX_im
                f_eq_im=2*f_eq-f_eq_old;
                FLX_im=2*FLX-FLX_old;
                %%%% Calculate the current pdf at cell centroid-Implicit scheme
                for r=1:M
                    P=CELL{r};
                    %% 1st Algorithm, fully implicit for collision term
                    f(:,r)=(f_old(:,r)-FLX_im(:,r)*dt/P{6}+f_eq_im(:,r)*dt/Tau)/(1+dt/Tau);
                    %% 2nd Algorithm, half implicit for collision term
                    %                 f(:,r)=((1-dt/Tau/2)*f_old(:,r)-(FLX_im(:,r)+FLX(:,r))*dt/2/P{6}+(f_eq_im(:,r)+f_eq(:,r))*dt/Tau/2)/(1+dt/Tau/2);
                end
                %%%%f_eq_old
                f_eq_old=f_eq;
                FLX_old=FLX;
            end
        elseif FT==3 % AB
            if tt==1 % Fisrt time step
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        % if W~=1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                        % end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%%% Update macro velocity and density at nodes
                [Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Solving governing equation
                %%%% Calculate flux integral for all triangles
                FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate collision integral over triangles
                for r=1:M
                    P=CELL{r};
                    FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
                end
                FC_old_old=FLX+FCOL;
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                for r=1:M
                    P=CELL{r};
                    f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(FCOL(:,r)-FLX(:,r));
                end
            else % Afterward
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        if W~=1 && FMP>1
                            %f_nd(:,l)=node_star(NP,f_old,0);
                        end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%%% Update macro velocity and density at nodes
                [Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
                %%%% Solving governing equation
                %%%% Calculate flux integral for all triangles
                FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate collision integral over triangles
                for r=1:M
                    P=CELL{r};
                    FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
                end
                FC_old=FLX+FCOL;
                %%%% Calculate the current pdf at cell centroid/// See derivation in
                %%%% notes
                for r=1:M
                    P=CELL{r};
                    %% Algorithm 1
                    %                 f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/2/P{6}/W*(3*FC_old(:,r)-FC_old_old(:,r));
                    %% Algorithm 2
                    f(:,r)=f_old(:,r)+(1-W)/3/W*(f_nd_old(:,P{7})+f_nd_old(:,P{8})+f_nd_old(:,P{9})-f_nd(:,P{7})-f_nd(:,P{8})-f_nd(:,P{9}))+dt/P{6}/W*(2*FC_old(:,r)-FC_old_old(:,r));
                end
                %%%% Update
                FC_old_old=FC_old;
                f_old_old=f_old;
            end
        elseif FT==4 % RK2
            %% 1st algorithm for RK2
            %%%%%% Update macro velocity and density at cell centroid
            [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    if W~=1 && FMP>1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                    end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Solving governing equation
            %%%% Calculate flux integral for all triangles
            FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %%%% Calculate collision integral over triangles
            for r=1:M
                P=CELL{r};
                FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            end
            %%%% Calculate the current pdf at cell centroid/// See derivation in
            %%%% notes
            for r=1:M
                P=CELL{r};
                f_K1(:,r)=f_old(:,r)+dt/P{6}*(FCOL(:,r)-FLX(:,r));
            end
            %%%%%% Update macro velocity and density at cell centroid
            [Rho_K1,U_K1,T_K1]=macro_h(f_K1,V,Rho_ref,FD);
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq_K1(:,r)=eqm_h(V,U_K1(:,r),Rho_K1(1,r),T_K1(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%% Update macro velocity and density at nodes
            Rho_nd_K1=Rho_nd;
            U_nd_K1=U_nd;
            f_nd_K1=f_nd;
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    if W~=1 && FMP>1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                    end
                else % nodes on boundary
                    f_nd_K1(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_K1,f_eq_K1,f_nd_K1,U_nd_K1,U_K1,Rho_nd_K1,Rho_K1,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd_K1,U_nd_K1,T_nd_K1]=macro_h(f_nd_K1,V,Rho_ref,FD);
            %%%% Solving governing equation
            %%%% Calculate flux integral for all triangles
            FLX_K1=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd_K1,f_K1,f_eq_K1,Rho_K1,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %%%% Calculate collision integral over triangles
            for r=1:M
                P=CELL{r};
                FCOL_K1(:,r)=BGK(qh,P,W,Tau,f_K1(:,r),f_nd_K1(:,P{7}),f_nd_K1(:,P{8}),f_nd_K1(:,P{9}),f_eq_K1(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            end
            %%%% Calculate the current pdf at cell centroid/// See derivation in
            %%%% notes
            for r=1:M
                P=CELL{r};
                f(:,r)=f_old(:,r)/2+f_K1(:,r)/2+dt/2/P{6}*(FCOL_K1(:,r)-FLX_K1(:,r));
            end
            
            %% 2nd algorithm for RK2
            %         %%%%%% Update macro velocity and density at cell centroid
            %         [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
            %         %%%% Calculate equilibrium pdf at cell centroids
            %         for r=1:M
            %             f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            %         end
            %         %%%%%%Nodal pdf updated and boundary conditions applied
            %         for l=1:N
            %             NP=NODE{l};
            %             if NP{2}==0 % Interior nodes
            %                 if W~=1 && FMP>1
            %                     %f_nd(:,l)=node_star(NP,f_old,0);
            %                 end
            %             else % nodes on boundary
            %                 f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
            %             end
            %         end
            %         %%%%%% Update macro velocity and density at nodes
            %         %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %         %%%% Solving governing equation
            %         %%%% Calculate flux integral for all triangles
            %         FLX=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %         %%%% Calculate collision integral over triangles
            %         for r=1:M
            %             P=CELL{r};
            %             FCOL(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            %         end
            %         %%%% Calculate f_K1
            %         %%%% notes
            %         for r=1:M
            %             P=CELL{r};
            %             f_K1(:,r)=f_old(:,r)+dt/P{6}/2*(FCOL(:,r)-FLX(:,r));
            %         end
            %         %%%%%% Update macro velocity and density at cell centroid
            %         [Rho_K1,U_K1,T_K1]=macro_h(f_K1,V,Rho_ref,FD);
            %         %%%% Calculate equilibrium pdf at cell centroids
            %         for r=1:M
            %             f_eq_K1(:,r)=eqm_h(V,U_K1(:,r),Rho_K1(1,r),T_K1(1,r),qh,wh,Rho_ref,FD);
            %         end
            %         %%%%%% Update macro velocity and density at nodes
            %         Rho_nd_K1=Rho_nd;
            %         U_nd_K1=U_nd;
            %         f_nd_K1=f_nd;
            %         %%%%%%Nodal pdf updated and boundary conditions applied
            %         for l=1:N
            %             NP=NODE{l};
            %             if NP{2}==0 % Interior nodes
            %                 if W~=1 && FMP>1
            %                     %f_nd(:,l)=node_star(NP,f_old,0);
            %                 end
            %             else % nodes on boundary
            %                 f_nd_K1(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,f_K1,f_eq_K1,f_nd_K1,U_nd_K1,U_K1,Rho_nd_K1,Rho_K1,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
            %             end
            %         end
            %         %%%%%% Update macro velocity and density at nodes
            %         %[Rho_nd_K1,U_nd_K1,T_nd_K1]=macro_h(f_nd_K1,V,Rho_ref,FD);
            %         %%%% Solving governing equation
            %         %%%% Calculate flux integral for all triangles
            %         FLX_K1=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd_K1,f_K1,f_eq_K1,Rho_K1,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %         %%%% Calculate collision integral over triangles
            %         for r=1:M
            %             P=CELL{r};
            %             FCOL_K1(:,r)=BGK(qh,P,W,Tau,f_K1(:,r),f_nd_K1(:,P{7}),f_nd_K1(:,P{8}),f_nd_K1(:,P{9}),f_eq_K1(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            %         end
            %         %%%% Calculate the current pdf at cell centroid/// See derivation in
            %         %%%% notes
            %         for r=1:M
            %             P=CELL{r};
            %             f(:,r)=f_old(:,r)+dt/P{6}*(FCOL_K1(:,r)-FLX_K1(:,r));
            %         end
        elseif FT==5 % RK4
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%% Update macro velocity and density at cell centroid
            [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    if W~=1 && FMP>1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                    end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Solving governing equation
            %%%% Calculate flux integral for all triangles
            FLX_K1=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %%%% Calculate collision integral over triangles
            for r=1:M
                P=CELL{r};
                FCOL_K1(:,r)=BGK(qh,P,W,Tau,f_old(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            end
            for r=1:M
                P=CELL{r};
                K1(:,r)=(FCOL_K1(:,r)-FLX_K1(:,r))/P{6};
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            f_K1=f_old+dt/2*K1;
            
            [Rho_K1,U_K1,T_K1]=macro_h(f_K1,V,Rho_ref,FD);
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq_K1(:,r)=eqm_h(V,U_K1(:,r),Rho_K1(1,r),T_K1(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N;
                NP=NODE{l};
                if NP{2}==0; % Interior nodes
                    if W~=1 && FMP>1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                    end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_K1,f_eq_K1,f_nd,U_nd,U_K1,Rho_nd,Rho_K1,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Solving governing equation
            %%%% Calculate flux integral for all triangles
            FLX_K2=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_K1,f_eq_K1,Rho_K1,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %%%% Calculate collision integral over triangles
            for r=1:M
                P=CELL{r};
                FCOL_K2(:,r)=BGK(qh,P,W,Tau,f_K1(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq_K1(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            end
            for r=1:M
                P=CELL{r};
                K2(:,r)=(FCOL_K2(:,r)-FLX_K2(:,r))/P{6};
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K3 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            f_K2=f_old+dt/2*K2;
            
            [Rho_K2,U_K2,T_K2]=macro_h(f_K2,V,Rho_ref,FD);
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq_K2(:,r)=eqm_h(V,U_K2(:,r),Rho_K2(1,r),T_K2(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    if W~=1 && FMP>1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                    end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_K2,f_eq_K2,f_nd,U_nd,U_K2,Rho_nd,Rho_K2,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Solving governing equation
            %%%% Calculate flux integral for all triangles
            FLX_K3=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_K2,f_eq_K2,Rho_K2,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %%%% Calculate collision integral over triangles
            for r=1:M
                P=CELL{r};
                FCOL_K3(:,r)=BGK(qh,P,W,Tau,f_K2(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq_K2(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            end
            for r=1:M
                P=CELL{r};
                K3(:,r)=(FCOL_K3(:,r)-FLX_K3(:,r))/P{6};
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K3 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K4 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            f_K3=f_old+dt*K3;
            
            [Rho_K3,U_K3,T_K3]=macro_h(f_K3,V,Rho_ref,FD);
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq_K3(:,r)=eqm_h(V,U_K3(:,r),Rho_K3(1,r),T_K3(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    if W~=1 && FMP>1
                        %f_nd(:,l)=node_star(NP,f_old,0);
                    end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_K3,f_eq_K3,f_nd,U_nd,U_K3,Rho_nd,Rho_K3,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Solving governing equation
            %%%% Calculate flux integral for all triangles
            FLX_K4=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_K3,f_eq_K3,Rho_K3,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %%%% Calculate collision integral over triangles
            for r=1:M
                P=CELL{r};
                FCOL_K4(:,r)=BGK(qh,P,W,Tau,f_K3(:,r),f_nd(:,P{7}),f_nd(:,P{8}),f_nd(:,P{9}),f_eq_K3(:,r),f_nd_eq(:,P{7}),f_nd_eq(:,P{8}),f_nd_eq(:,P{9}));
            end
            for r=1:M
                P=CELL{r};
                K4(:,r)=(FCOL_K4(:,r)-FLX_K4(:,r))/P{6};
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% K4 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            FLX=(FLX_K1+FLX_K2+FLX_K3+FLX_K4)/4;
            f=f_old+dt*(K1/6+K2/3+K3/3+K4/6);
        elseif FT==6 % Semi-Langragian implicit f_eq only, still Eulerian advection
            %%%%%% Update macro velocity and density at cell centroid
            [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
            if FTH==1 && qh~=37
                T=macro_t(g_old,V2,Rho);
            end
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    %                     if W~=1 && FMP>1
                    %                         %f_nd(:,l)=node_star(NP,f_old,0);
                    %                     end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%% Nodal variable update
            % if W~=1
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Calculate flux integral for all triangles
            if tt==1
                pdf_h_face_center_old=zeros(qh,O);
                for r=1:O
                    FC=FACE{r};
                    pdf_h_face_center_old(:,r)=(f_nd(:,FC{8})+f_nd(:,FC{9}))/2;
                end
            else
                pdf_h_face_center_old=pdf_h_face_center_new;
            end
            [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            %             if FInr==1 || FInr==9 % Godunov flux, requires different time marching
            %                 for r=1:M
            %                     P=CELL{r};
            %                     f(:,r)=(f_old(:,r)*(2*Tau-dt)+dt*(3*f_eq(:,r)-f_eq_old(:,r))-FLX(:,r)*2*Tau*dt/P{6})/(2*Tau+dt); % Scheme 1, linear interpolation of collision term, this proved to be the same as the IC-EA scheme
            % %                     f(:,r)=(1-exp(-dt/Tau))*f_eq(:,r)+exp(-dt/Tau)*f_old(:,r)-FLX(:,r)*dt/P{6}; % Scheme 2, exp decaying of collision term
            %                 end
            %             else
            %%%% Calculate the implicit f_eq with Lagrangian method
            %                 f_eq_im=eqm_h_lagrangian(CELL,M,f_old,f_nd,f_eq,V,V1,V2,qh,wh,Rho_ref,FMP,FPDC,FD);
            %% The following line replaces the previous line for simpicity
            if FSLIC==0
                f_eq_im=f_eq;
            elseif FSLIC==1
                f_im=in_cell_mapping_SLIC_apply(CELL,V,f_old,f_eq,f_nd,Tau,FPDC);
                
                [Rho_im,U_im,T_im]=macro_h(f_im,V,Rho_ref,FD);
%                 if FTH==1 && qh~=37
%                     T=macro_t(g_old,V2,Rho);
%                 end
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq_im(:,r)=eqm_h(V,U_im(:,r),Rho_im(1,r),T_im(1,r),qh,wh,Rho_ref,FD);
                end
            else
                error('Wrong flag for SLIC scheme!');
            end
            %%%% Calculate the current pdf at cell centroid-Implicit scheme
            if FFC==0
                if FInr==1 || FInr==9
                    %% 2nd Algorithm, half implicit for collision term
                    f=((1-dt/Tau/2)*f_old-FLX*dt./Vol+(f_eq_im+f_eq)*dt/Tau/2)/(1+dt/Tau/2);
                else
                    %% 1st Algorithm, fully implicit for collision term
                    f=(f_old-FLX*dt./Vol+f_eq_im*dt/Tau)/(1+dt/Tau);
                end
            elseif FFC==1
                FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                if FInr==1 || FInr==9
                    %% 2nd Algorithm, half implicit for collision term
                    f=((1-dt/Tau/2)*f_old-FLX*dt./Vol+(f_eq_im+f_eq)*dt/Tau/2+FORCE*dt/2)/(1+dt/Tau/2);
                else
                    %% 1st Algorithm, fully implicit for collision term
                    f=(f_old-FLX*dt./Vol+f_eq_im*dt/Tau+dt*FORCE)/(1+dt/Tau);
                end
            else
                error('Wrong flag for force term!');
            end
        elseif FT==7 % Variable transformation
            %%%%%% Update macro velocity and density at cell centroid
            [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
            if FTH==1 && qh~=37
                T=macro_t(g_old,V2,Rho);
            end
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    %                     if W~=1 && FMP>1
                    %                         %f_nd(:,l)=node_star(NP,f_old,0);
                    %                     end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%% Nodal variable update
            % if W~=1
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Calculate flux integral for all triangles
            if tt==1
                pdf_h_face_center_old=zeros(qh,O);
                for r=1:O
                    FC=FACE{r};
                    pdf_h_face_center_old(:,r)=(f_nd(:,FC{8})+f_nd(:,FC{9}))/2;
                end
            else
                pdf_h_face_center_old=pdf_h_face_center_new;
            end
            [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            
            %%%% Calculate the current pdf at cell centroid-Implicit scheme
            if FFC==0
                if FInr==1 || FInr==9
                    %% 2nd Algorithm, half implicit for collision term
                    f=((2*Tau-dt)/Tau/2)*f_old-FLX./Vol*dt+f_eq*dt/Tau/2;
                else
                    %% 1st Algorithm, fully implicit for collision term
                    f=f_old-FLX./Vol*dt;
                end
            elseif FFC==1
                FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        f=((2*Tau-dt)/Tau/2)*f_old-FLX./Vol*dt+f_eq*dt/Tau/2+FORCE*dt/2;
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        f=f_old-FLX./Vol*dt+dt*FORCE;
                    end
            else
                error('Wrong flag for force term!');
            end
            [Rho,U,T]=macro_h(f,V,Rho_ref,FD);
            for r=1:M
                f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            end
            if FInr==1 || FInr==9
                f=2*Tau/(dt+2*Tau)*(f+f_eq*dt/2/Tau);
            else
                f=Tau/(dt+Tau)*(f+f_eq*dt/Tau);
            end
        elseif FT==8 % Variable Transformation modified by the Semi-Lagrangian
            %%%%%% Update macro velocity and density at cell centroid
            [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
            if FTH==1 && qh~=37
                T=macro_t(g_old,V2,Rho);
            end
            %%%% Calculate equilibrium pdf at cell centroids
            for r=1:M
                f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            end
            %%%%%%Nodal pdf updated and boundary conditions applied
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                    %                     if W~=1 && FMP>1
                    %                         %f_nd(:,l)=node_star(NP,f_old,0);
                    %                     end
                else % nodes on boundary
                    f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                end
            end
            %%%%% Nodal variable update
            % if W~=1
            %%%%%% Update macro velocity and density at nodes
            %[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
            %%%% Calculate flux integral for all triangles
            if tt==1
                pdf_h_face_center_old=zeros(qh,O);
                for r=1:O
                    FC=FACE{r};
                    pdf_h_face_center_old(:,r)=(f_nd(:,FC{8})+f_nd(:,FC{9}))/2;
                end
            else
                pdf_h_face_center_old=pdf_h_face_center_new;
            end
            [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
            
            %%%% Calculate the current pdf at cell centroid-Implicit scheme
            if FFC==0
                if FInr==1 || FInr==9
                    %% 2nd Algorithm, half implicit for collision term
                    f=((2*Tau-dt)/Tau/2)*f_old-FLX./Vol*dt+f_eq*dt/Tau/2;
                else
                    %% 1st Algorithm, fully implicit for collision term
                    f=f_old-FLX./Vol*dt;
                end
            elseif FFC==1
                FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                if FInr==1 || FInr==9
                    %% 2nd Algorithm, half implicit for collision term
                    f=((2*Tau-dt)/Tau/2)*f_old-FLX./Vol*dt+f_eq*dt/Tau/2+FORCE*dt/2;
                else
                    %% 1st Algorithm, fully implicit for collision term
                    f=f_old-FLX./Vol*dt+dt*FORCE;
                end
            else
                error('Wrong flag for force term!');
            end
            %             [Rho,U,T]=macro_h(f,V,Rho_ref,FD);
            %             for r=1:M
            %                 f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
            %             end
            if FInr==1 || FInr==9
                f=2*Tau/(dt+2*Tau)*(f+f_eq*dt/2/Tau);
            else
                f=Tau/(dt+Tau)*(f+f_eq*dt/Tau);
            end
        elseif FT==9 % Solving f_eq_im iteratively
            while RE>1e-6
                %%%%%% Update macro velocity and density at cell centroid
                [Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
                if FTH==1 && qh~=37
                    T=macro_t(g_old,V2,Rho);
                end
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        %                     if W~=1 && FMP>1
                        %                         %f_nd(:,l)=node_star(NP,f_old,0);
                        %                     end
                    else % nodes on boundary
                        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                if tt==1
                    pdf_h_face_center_old=zeros(qh,O);
                    for r=1:O
                        FC=FACE{r};
                        pdf_h_face_center_old(:,r)=(f_nd(:,FC{8})+f_nd(:,FC{9}))/2;
                    end
                else
                    pdf_h_face_center_old=pdf_h_face_center_new;
                end
                [FLX,pdf_h_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,f_nd,f_old,f_eq,pdf_h_face_center_old,Rho,Rho_ref,V,dt,wl,wlb,wh,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %             if FInr==1 || FInr==9 % Godunov flux, requires different time marching
                %                 for r=1:M
                %                     P=CELL{r};
                %                     f(:,r)=(f_old(:,r)*(2*Tau-dt)+dt*(3*f_eq(:,r)-f_eq_old(:,r))-FLX(:,r)*2*Tau*dt/P{6})/(2*Tau+dt); % Scheme 1, linear interpolation of collision term, this proved to be the same as the IC-EA scheme
                % %                     f(:,r)=(1-exp(-dt/Tau))*f_eq(:,r)+exp(-dt/Tau)*f_old(:,r)-FLX(:,r)*dt/P{6}; % Scheme 2, exp decaying of collision term
                %                 end
                %             else
                %%%% Calculate the extrapolated f_eq_im
                f_eq_old=f_eq;
                %%%% Calculate the current pdf at cell centroid-Implicit scheme
                if FFC==0
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        f=((1-dt/Tau/2)*f_old-FLX*dt./Vol+(f_eq_old+f_eq)*dt/Tau/2)/(1+dt/Tau/2);
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        f=(f_old-FLX*dt./Vol+f_eq_old*dt/Tau)/(1+dt/Tau);
                    end
                elseif FFC==1
                    FORCE=force(V,wh,Rho,T,T_ref,Body_force,FFC,FTH);
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        f=((1-dt/Tau/2)*f_old-FLX*dt./Vol+(f_eq_old+f_eq)*dt/Tau/2+FORCE*dt/2)/(1+dt/Tau/2);
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        f=(f_old-FLX*dt./Vol+f_eq_old*dt/Tau+dt*FORCE)/(1+dt/Tau);
                    end
                else
                    error('Wrong flag for force term!');
                end
                
                [Rho,U,T]=macro_h(f,V,Rho_ref,FD);
                if FTH==1 && qh~=37
                    T=macro_t(g_old,V2,Rho);
                end
                %%%% Calculate equilibrium pdf at cell centroids
                for r=1:M
                    f_eq_new(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
                end
                RE=sum(sum(f_eq_new-f_eq_old,2));
                f_old=f;
            end
        else
            error('The flag for time-matching scheme is unavailable or invalid!');
        end
    end
    
    %% Thermal model
%     pdf_g_face_center_old=zeros(qh,O);
%     for r=1:O
%         FC=FACE{r};
%         pdf_g_face_center_old(:,r)=(g_nd(:,FC{8})+g_nd(:,FC{9}))/2;
%     end
%     pdf_g_face_center_new=pdf_g_face_center_old;
    if FTH==1
        if FTHM==0 % Passive scalar
            if FT==1 % Implicit Euler, CI/AE
                %%%%%% Update macro temperature at cell centroid
                T=macro_t(g_old,V2,Rho);
                %%%% Calculate equilibrium pdf at cell centroids
%                 if qt==qh
%                     for r=1:M
%                         g_eq(:,r)=T(1,r)*f_eq(:,r);
%                     end
%                 else
                    for r=1:M
                        g_eq(:,r)=eqm_t(V2,U(:,r),Rho(1,r),T(1,r),qt,wt,Rho_ref,FD);
                    end
%                 end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        ;
                    else % nodes on boundary
                        g_nd(:,l)=pdf_bc_t(N_L,N_H,N_I,X1,X2,Y1,Y2,Tau_t,dt,CELL,NP,f_old,f_eq,f_nd,g_old,g_eq,g_nd,U_nd,U,Rho_nd,Rho,T,V2,V1,V2,Rho_ref,Rho_in,Rho_out,qh,qt,wc,wh,wt,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                %%%%% Nodal variable update
                %%%% Calculate flux integral for all triangles
                if tt==1
                    %%%%%% Initiate the value of pdf_face_center_old
                    pdf_g_face_center_old=zeros(qt,O);
                    for r=1:O
                        FC=FACE{r};
                        pdf_g_face_center_old(:,r)=(g_nd(:,FC{8})+g_nd(:,FC{9}))/2;
                    end
                else
                    pdf_g_face_center_old=pdf_g_face_center_new;
                end
                [FLX_t,pdf_g_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,g_nd,g_old,g_eq,pdf_g_face_center_old,Rho,Rho_ref,V2,dt,wl,wlb,wt,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
%                 FLX_t=flux_pdf_h(CELL,M,NODE,N,FACE,O,g_nd,g_old,g_eq,Rho,Rho_ref,V2,dt,wl,wlb,wt,V1,V2,0,0,0,0,0,0,0,0,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
%                 FLX_t=flux_pdf_h(CELL,M,NODE,N,FACE,O,g_nd,g_old,g_eq,Rho,Rho_ref,V2,dt,wl,wlb,wt,V1,V2,1,1,1,1,1,1,1,1,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate the extrapolated f_eq_im
                g_eq_im=2*g_eq-g_eq_old;
                %%%% Calculate the current pdf at cell centroid-Implicit scheme
                for r=1:M
                    P=CELL{r};
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        g(:,r)=((1-dt/Tau_t/2)*g_old(:,r)-FLX_t(:,r)*dt/P{6}+(g_eq_im(:,r)+g_eq(:,r))*dt/Tau_t/2)/(1+dt/Tau_t/2);
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        g(:,r)=(g_old(:,r)-FLX_t(:,r)*dt/P{6}+g_eq_im(:,r)*dt/Tau_t)/(1+dt/Tau_t);
                    end
                end
                %%%%f_eq_old
                g_eq_old=g_eq;
            elseif FT==6
                %%%%%% Update macro temperature at cell centroid
                T=macro_t(g_old,V2,Rho);
                %%%% Calculate equilibrium pdf at cell centroids
%                 if qt==qh
%                     for r=1:M
%                         g_eq(:,r)=T(1,r)*f_eq(:,r);
%                     end
%                 else
                    for r=1:M
                        g_eq(:,r)=eqm_t(V2,U(:,r),Rho(1,r),T(1,r),qt,wt,Rho_ref,FD);
                    end
%                 end
                %%%%%%Nodal pdf updated and boundary conditions applied
                for l=1:N
                    NP=NODE{l};
                    if NP{2}==0 % Interior nodes
                        ;
                    else % nodes on boundary
                        g_nd(:,l)=pdf_bc_t(N_L,N_H,N_I,X1,X2,Y1,Y2,Tau_t,dt,CELL,NP,f_old,f_eq,f_nd,g_old,g_eq,g_nd,U_nd,U,Rho_nd,Rho,T,V2,V1,V2,Rho_ref,Rho_in,Rho_out,qh,qt,wc,wh,wt,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
                    end
                end
                if tt==1
                    %%%%%% Initiate the value of pdf_face_center_old
                    pdf_g_face_center_old=zeros(qt,O);
                    for r=1:O
                        FC=FACE{r};
                        pdf_g_face_center_old(:,r)=(g_nd(:,FC{8})+g_nd(:,FC{9}))/2;
                    end
                else
                    pdf_g_face_center_old=pdf_g_face_center_new;
                end
                %%%%% Nodal variable update
                %%%% Calculate flux integral for all triangles
                [FLX_t,pdf_g_face_center_new]=flux_pdf_h(CELL,M,NODE,N,FACE,O,g_nd,g_old,g_eq,pdf_g_face_center_old,Rho,Rho_ref,V2,dt,wl,wlb,wt,V1,V2,FInr,FPdc,FInt,FOut,FStw,FMow,FWdp,FIof,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
%                 FLX_t=flux_pdf_h(CELL,M,NODE,N,FACE,O,g_nd,g_old,g_eq,Rho,Rho_ref,V2,dt,wl,wlb,wt,V1,V2,0,0,0,0,0,0,0,0,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
%                 FLX_t=flux_pdf_h(CELL,M,NODE,N,FACE,O,g_nd,g_old,g_eq,Rho,Rho_ref,V2,dt,wl,wlb,wt,V1,V2,1,1,1,1,1,1,1,1,FUPD,FTVD,FPDC,FD,FM,FMP,FE);
                %%%% Calculate the extrapolated f_eq_im
                g_eq_im=eqm_t_lagrangian(CELL,M,g_old,g_nd,g_eq,V,V1,V2,qt,wt,U,Rho,Rho_ref,FMP,FPDC,FD);
                %%%% Calculate the current pdf at cell centroid-Implicit scheme
                for r=1:M
                    P=CELL{r};
                    if FInr==1 || FInr==9
                        %% 2nd Algorithm, half implicit for collision term
                        g(:,r)=((1-dt/Tau_t/2)*g_old(:,r)-FLX_t(:,r)*dt/P{6}+(g_eq_im(:,r)+g_eq(:,r))*dt/Tau_t/2)/(1+dt/Tau_t/2);
                    else
                        %% 1st Algorithm, fully implicit for collision term
                        g(:,r)=(g_old(:,r)-FLX_t(:,r)*dt/P{6}+g_eq_im(:,r)*dt/Tau_t)/(1+dt/Tau_t);
                    end
                end
                %%%%f_eq_old
                g_eq_old=g_eq;
            else
                error('The flag for time-matching scheme is unavailable or invalid for Passive-Scalar thermal model!');
            end
        elseif FTHM==1 % double distribution
            error('Temporarily not available!');
        elseif FTHM==2 % High order lattice
            if qh~=37
                error('Wrong lattice type for high order lattice thermal model!');
            end
        else
            error('Wrong flag for thermal model type!');
        end
        g_old=g;
        g_nd_old=g_nd;
    end
    %%%% Monitored variables
    if FTH==0 || (FTH==1 && FCD==0)
        R(tt+1)=norm(f-f_old,'fro');
    end
    %R(tt+1)=norm(f-f_eq,'fro');
    RHO(tt+1)=0;
    for r=1:M
        RHO(tt+1)=RHO(tt+1)+Rho(1,r);
    end
    RHO(tt+1)=RHO(tt+1)/M;
    
    UR(:,tt+1)=[0;0];
    for r=1:M
        UR(:,tt+1)=UR(:,tt+1)+U(:,r);
    end
    UR(:,tt+1)=UR(:,tt+1)/M;
    
    TR(tt+1)=0;
    for r=1:M
        TR(tt+1)=TR(tt+1)+T(1,r);
    end
    TR(tt+1)=TR(tt+1)/M;
    
    if FTH==0 || (FTH==1 && FCD==0)
        FLX_c(1:qh,tt+1)=0;
        for k=1:qh
            for r=1:M
                FLX_c(k,tt+1)=FLX_c(k,tt+1)+FLX(k,r);
            end
        end
        FLX_c(1:qh,tt+1)=FLX_c(1:qh,tt+1)/M;
    end
    
    %% Track the history of f_eq^n+1-f_eq^n
    if i==1
        FEQ_Diff_DBM=zeros(qh,1);
    else
        FEQ_Diff_DBM=[FEQ_Diff_DBM,zeros(qh,1)];
    end
    
    if i>1
        FEQ_Diff_DBM(:,end)=sum(f_eq-f_eq_track_old,2)/M;
    end
    f_eq_track_old=f_eq;
    %     FCOL_c(1:qh,tt+1)=0;
    %     for k=1:qh
    %         for r=1:M
    %             FCOL_c(k,tt+1)=FCOL_c(k,tt+1)+FCOL(k,r);
    %         end
    %     end
    %     FCOL_c(1:qh,tt+1)=FCOL_c(1:qh,tt+1)/M;
    %     if FM==0
    %         for l=1:a
    %             RHO_NDX(1+2*(l-1):2*l,tt+1)=[Rho_nd(NDX(1,l));Rho_nd(NDX(2,l))];
    %             RHO_NDY(1+2*(l-1):2*l,tt+1)=[Rho_nd(NDY(1,l));Rho_nd(NDY(2,l))];
    %             U_NDX(1+4*(l-1):4*l,tt+1)=[U_nd(:,NDX(1,l));U_nd(:,NDX(2,l))];
    %             U_NDY(1+4*(l-1):4*l,tt+1)=[U_nd(:,NDY(1,l));U_nd(:,NDY(2,l))];
    %             if FF==1 % Taylor vortex flow
    %                 U_TV_NDX(1+4*(l-1):4*l,tt+1)=[-U_0*exp(-Tau/3*dt*tt*(k1^2+k2^2))*cos(k1*X(NDX(1,l)))*sin(k2*Y(NDX(1,l)));
    %                     U_0*(k1/k2)*exp(-Tau/3*dt*tt*(k1^2+k2^2))*sin(k1*X(NDX(1,l)))*cos(k2*Y(NDX(1,l)));
    %                     -U_0*exp(-Tau/3*dt*tt*(k1^2+k2^2))*cos(k1*X(NDX(2,l)))*sin(k2*Y(NDX(2,l)));
    %                     U_0*(k1/k2)*exp(-Tau/3*dt*tt*(k1^2+k2^2))*sin(k1*X(NDX(2,l)))*cos(k2*Y(NDX(2,l)));];
    %                 U_TV_NDY(1+4*(l-1):4*l,tt+1)=[-U_0*exp(-Tau/3*dt*tt*(k1^2+k2^2))*cos(k1*X(NDY(1,l)))*sin(k2*Y(NDY(1,l)));
    %                     U_0*(k1/k2)*exp(-Tau/3*dt*tt*(k1^2+k2^2))*sin(k1*X(NDY(1,l)))*cos(k2*Y(NDY(1,l)));
    %                     -U_0*exp(-Tau/3*dt*tt*(k1^2+k2^2))*cos(k1*X(NDY(2,l)))*sin(k2*Y(NDY(2,l)));
    %                     U_0*(k1/k2)*exp(-Tau/3*dt*tt*(k1^2+k2^2))*sin(k1*X(NDY(2,l)))*cos(k2*Y(NDY(2,l)));];
    %             end
    %         end
    %         if FS==0
    %             FCOL_c(1:qh,tt+1)=0;
    %             for k=1:qh
    %                 for r=1:M
    %                     FCOL_c(k,tt+1)=FCOL_c(k,tt+1)+FCOL(k,r);
    %                 end
    %             end
    %             FCOL_c(1:qh,tt+1)=FCOL_c(1:qh,tt+1)/M;
    %
    %             for l=1:a
    %                 RHO_NDX(1+2*(l-1):2*l,tt+1)=[Rho_nd(NDX(1,l));Rho_nd(NDX(2,l))];
    %                 RHO_NDY(1+2*(l-1):2*l,tt+1)=[Rho_nd(NDY(1,l));Rho_nd(NDY(2,l))];
    %                 U_NDX(1+4*(l-1):4*l,tt+1)=[U_nd(:,NDX(1,l));U_nd(:,NDX(2,l))];
    %                 U_NDY(1+4*(l-1):4*l,tt+1)=[U_nd(:,NDY(1,l));U_nd(:,NDY(2,l))];
    %             end
    %         end
    %     end
    %%%% Update pdf at cell centroids and nodes
    if FTH==0 || (FTH==1 && FCD==0)
        f_old=f;
        f_nd_old=f_nd;
    end
    %%%% Timer evolution
    tt=tt+1;
    TT(tt)=tt;
    %% Update tracer
    if FTRC==1
        for l=1:N
            NP=NODE{l};
            if NP{2}==0 % Interior nodes
                % if W~=1
%                 f_nd(:,l)=node_star(NP,f_old,0);
                % end
            else % nodes on boundary
                U_nd(:,l)=(V*f_nd(:,l))/sum(f_nd(:,l));
            end
        end
        trc.U=U;
        trc.U_nd=U_nd;
        trc.dt=dt;
        trc.update;
        if mod(i,Tracer_plot_interval)==0
            trc.FIG_COUNTER=trc.FIG_COUNTER+1;
            trc.showsave;
        end
    end
    %% Save Other animation plot
    if FANI==1
        if mod(i,Anim_plot_interval)==0
            FIG_COUNTER=FIG_COUNTER+1;
            for l=1:N
                NP=NODE{l};
                if NP{2}==0 % Interior nodes
                else % nodes on boundary
                    U_nd(:,l)=(V*f_nd(:,l))/sum(f_nd(:,l));
                end
            end
            for r=1:M
                P=CELL{r};
                U_M(r)=sqrt(U(1,r)^2+U(2,r)^2);
            end
            for r=1:N-N_I
                U_M(M+r)=sqrt(U_nd(1,r+N_I)^2+U_nd(2,r+N_I)^2);
            end
            figure(136);
            clf;
            Z=griddata(XXX,YYY,U_M,Xx,Yy);
            contourf(Xx,Yy,Z,300,'LineStyle','none');
            axis equal tight;
            set(gca,'xtick',[]);
            set(gca,'ytick',[]);
            set(gcf,'color','w');
            filename =['U_mag',num2str(FIG_COUNTER,'%04d')];
            saveas(gca,fullfile(pathname, filename),'jpg')
            print('-dtiff','-r300',filename)
        end
    end
end
toc;
Time_per_iter=toc/i
Time_per_iter_per_cell=Time_per_iter/M
%%%%%%Nodal pdf updated and boundary conditions applied
[Rho,U,T]=macro_h(f_old,V,Rho_ref,FD);
if FFC==1
    U=U+Body_force/2;
end
%%%% Calculate equilibrium pdf at cell centroids
for r=1:M
    f_eq(:,r)=eqm_h(V,U(:,r),Rho(1,r),T(1,r),qh,wh,Rho_ref,FD);
end
for l=1:N
    NP=NODE{l};
    if NP{2}==0 % Interior nodes
        % if W~=1
        f_nd(:,l)=node_star(NP,f_old,0);
        % end
    else % nodes on boundary
        f_nd(:,l)=pdf_bc_h(N_L,N_H,N_I,Tau,dt,NODE,NP,CELL,f_old,f_eq,f_nd,U_nd,U,Rho_nd,Rho,V,V1,V2,Rho_ref,Rho_in,Rho_out,qh,wc,wh,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
    end
end
[Rho_nd,U_nd,T_nd]=macro_h(f_nd,V,Rho_ref,FD);
if FFC==1
    U_nd=U_nd+Body_force/2;
end
if FTH==1 && FTHM~=2
    T=macro_t(g_old,V2,Rho);
    %%%% Calculate equilibrium pdf at cell centroids
%     if qt==qh
%         for r=1:M
%             g_eq(:,r)=T(1,r)*f_eq(:,r);
%         end
%     else
        for r=1:M
            g_eq(:,r)=eqm_t(V2,U(:,r),Rho(1,r),T(1,r),qt,wt,Rho_ref,FD);
        end
%     end
    for l=1:N
        NP=NODE{l};
        if NP{2}==0 % Interior nodes
            g_nd(:,l)=node_star(NP,g_old,0);
        else % nodes on boundary
            g_nd(:,l)=pdf_bc_t(N_L,N_H,N_I,X1,X2,Y1,Y2,Tau_t,dt,CELL,NP,f_old,f_eq,f_nd,g_old,g_eq,g_nd,U_nd,U,Rho_nd,Rho,T,V2,V1,V2,Rho_ref,Rho_in,Rho_out,qh,qt,wc,wh,wt,FD,FDC,NPdc,NInt,NOut,NStw,NMow,NWdp,NIof,NC70,NC71,NC72,NC73,NC74,NC75,NC76,NC77,NC78,NC79,NC80);
        end
    end
    T_nd=macro_t(g_nd,V2,Rho_nd);
end
%%% Calculate the velocity megnitude in cell centroid
U_M=zeros(1,M);
for r=1:M
    P=CELL{r};
    U_M(r)=sqrt(U(1,r)^2+U(2,r)^2);
end
%%% Adding the boundary values in the matrices for display
XXX=zeros(1,M);
YYY=zeros(1,M);
%%%% coordinates
for r=1:M
    P=CELL{r};
    Centroid=P{5};
    XXX(r)=Centroid(1,1);
    YYY(r)=Centroid(2,1);
end
for r=1:N-N_I
    XXX(M+r)=X(r+N_I);
    YYY(M+r)=Y(r+N_I);
end
for r=1:N_I_N
    XXX(M+N-N_I+r)=X(r);
    YYY(M+N-N_I+r)=Y(r);
end
U_plt=U;
Rho_plt=Rho;
T_plt=T;
for r=1:N-N_I
    U_plt(:,M+r)=U_nd(:,r+N_I);
    U_M(M+r)=sqrt(U_nd(1,r+N_I)^2+U_nd(2,r+N_I)^2);
    Rho_plt(1,M+r)=Rho_nd(1,r+N_I);
    T_plt(1,M+r)=T_nd(1,r+N_I);
end
for r=1:N_I_N
    U_plt(:,M+N-N_I+r)=U_nd(:,r);
    U_M(M+N-N_I+r)=sqrt(U_nd(1,r)^2+U_nd(2,r)^2);
    Rho_plt(1,M+N-N_I+r)=Rho_nd(1,r);
    T_plt(1,M+N-N_I+r)=T_nd(1,r);
end